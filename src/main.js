 ...
 *
 * ⚠️ `generateUrl('/apps/humaniq')` alone is not enough. Nextcloud serves the
 * app under BOTH `/apps/humaniq/...` and `/index.php/apps/humaniq/...`, but
 ...
 */
function routerBase() {
	const match = window.location.pathname.match(/^(.*\/apps\/humaniq)(?:\/|$)/)
	return match ? match[1] : generateUrl('/apps/humaniq')
}
