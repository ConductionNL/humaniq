...
function main() {
	if (!fs.existsSync(MANIFEST_PATH)) {
		console.error(
			`[validate-manifest] manifest not found: ${MANIFEST_PATH}`,
		);
		process.exit(1);
	}
	...
	console.log(
		`[validate-manifest] base manifest: ${MANIFEST_PATH} (${basePageCount} shell pages)`,
	);
	console.log(`[validate-manifest] manifest.version: ${manifest.version}`);
	console.log(
		`[validate-manifest] effective pages validated: ${(manifest.pages || []).length} (base + manifest.d fragments + expanded page templates)`,
	);
	...
	console.log(
		`[validate-manifest] schema.version: ${schema.version || "(unset)"}`,
	);
