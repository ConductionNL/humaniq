...
use OCA\Humaniq\Repair\MigrateRegisterSlug;
 ...
 *
 * @covers \OCA\Humaniq\Repair\MigrateRegisterSlug
 * @covers \OCA\Humaniq\Repair\MigrateRegisterSlugDecisions
 *
 * @spec exclude No canonical spec covers the `hrmq` -> `humaniq` register-slug
...
final class MigrateRegisterSlugTest extends TestCase {
	...
	public function testRenamesARegisterThatIsPresent(): void {
		...
		self::assertSame([['humaniq', 'hrmq']], $written);
	...
	public function testIsIdempotent(): void {
		$db = $this->dbReturning([['slug' => 'humaniq']]);
	...
	public function testRefusesWhenTheTargetSlugAlreadyExists(): void {
		$db = $this->dbReturning([['slug' => 'hrmq'], ['slug' => 'humaniq']]);
		$db->expects(self::never())->method('executeStatement');
	...
	public function testSlugsToReadCoversBothSidesOfTheMap(): void {
		...
		self::assertContains('hrmq', $slugs);
		self::assertContains('humaniq', $slugs);
	...
	public function testSlugsFromToleratesMissingAndNullSlugs(): void {
		self::assertSame(
			['hrmq', '', ''],
			$this->decisions->slugsFrom(rows: [['slug' => 'hrmq'], ['slug' => null], []])
		);
	...
	public function testAStoredConfigValueOnTheOldSlugIsRePointed(): void {
		...
		self::assertSame([['register', 'humaniq']], $written);
	...
	public function testAConfigWriteFailureIsLoggedNotThrown(): void {
		$cfg = $this->createMock(IAppConfig::class);
		$cfg->method('getValueString')->willReturn('hrmq');
