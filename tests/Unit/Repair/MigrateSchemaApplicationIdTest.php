...
use OCA\Humaniq\Repair\MigrateSchemaApplicationId;
use OCP\DB\IResult;
 ...
 *
 * @covers \OCA\Humaniq\Repair\MigrateSchemaApplicationId
 *
...
final class MigrateSchemaApplicationIdTest extends TestCase {
	...
	public function testMovesASchemaThatHasNoTwin(): void {
		...
		self::assertSame(
			[['humaniq', 'hrmq', 'case'], ['humaniq', 'hrmq', 'task']],
			$written
		);
	...
	public function testRefusesASlugThatWouldCollide(): void {
		...
		$output = $this->createMock(IOutput::class);
		$output->expects(self::once())->method('info')->with(self::stringContains('1 schema(s) moved to humaniq, 1 refused'));

		$step->run($output);

		self::assertSame([['humaniq', 'hrmq', 'task']], $written);
