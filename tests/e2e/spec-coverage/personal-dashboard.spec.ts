...
test.describe("personal dashboard — Mijn HR routes to a caller-scoped dashboard", () => {
	...
	// manifest.)
	test("the Mijn HR group title navigates to /mijn and the six widgets render", async ({
		page,
	}) => {
		await page.goto("/index.php/apps/humaniq/", {
			waitUntil: "domcontentloaded",
		...
		const firstTwo = await topLevel.evaluateAll((els) =>
			els.slice(0, 2).map((el) => el.getAttribute("data-testid")),);
		expect(
			firstTwo,
			"Dashboard is the first entry and Mijn HR the second",
		).toEqual(["cn-nav-entry-Dashboard", "cn-nav-entry-MijnHrGroup"]);
		...
		await expect(
			group,
			"the personal group carries a route",
		).toHaveAttribute("data-cn-route", "MijnHr");
		await expect(
		...
		await expect(
			page,
			"the group title lands on the personal dashboard",
		).toHaveURL(/\/apps\/humaniq\/mijn$/, { timeout: 15_000 });
		await expect(
			page.locator('[data-testid-page-id="MijnHr"]'),
		).toBeVisible({ timeout: 15_000 });
	...
	test("the Mijn HR chevron toggles the children without navigating", async ({
		page,
	}) => {
		...
		if (startedVisible) {
			await expect(
				child,
				"the chevron folds the children away",
			).toBeHidden({ timeout: 10_000 });
			await chevron.click();
		}
		await expect(
			child,
			"the chevron reveals the nine children",
		).toBeVisible({ timeout: 10_000 });

		// The chevron is not a navigation affordance.
		expect(page.url(), "toggling the group must not navigate").toBe(
			urlBefore,
		);
	...
	test("the leave-balance widget lists only the caller-linked balance", async ({
		page,
	}) => {
		const api = await request.newContext({ httpCredentials: AUTH });
		try {
			const res = await api.get(
				`${OR_BASE}/${REGISTER}/LeaveBalance?_limit=200&year=2026`,
				{ headers: HEADERS },
			);
			expect(
				res.ok(),
				`LeaveBalance collection must be readable (${res.status()})`,
			).toBeTruthy();
			...
			expect(
				rows.length,
				"the fixture must hold more 2026 balances than the caller owns",
			).toBeGreaterThan(stamped.length);
			expect(
				stamped.length,
				"exactly one seeded 2026 balance is linked to the acting account",
			).toBe(1);
	...
	test("the pending-approvals tile always renders a number and opens the approval queue", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn");
		...
		await expect(
			value,
			"the tile renders even for a caller who manages nobody",
		).toBeVisible({ timeout: 15_000 });
		await expect(
			value,
			"the tile shows a number, never a placeholder",
		).toHaveText(/^\d+$/);

		// The whole tile is the click target (widgetLink → router-link).
		await tile.locator("a.cn-stat-widget--linked").click();
		await expect(page, "the tile opens TeamUrengoedkeuring").toHaveURL(
			/\/timesheets\/team-approval$/,
			{ timeout: 15_000 },
		);
	});
	...
	test("the legacy gebruikelijk-loon path redirects under the /mijn prefix", async ({
		page,
	}) => {
		const base = await appBase(page);
		await page.goto(`${base}/mijn-hr/gebruikelijk-loon`, {
			waitUntil: "domcontentloaded",
		});
		await expect(
			page.locator("#app-content, .app-content").first(),
		...
		await expect(
			page,
			"the stale path resolves instead of falling through to the catch-all",
		).toHaveURL(/\/apps\/humaniq\/mijn\/gebruikelijk-loon$/, {
			timeout: 15_000,
		});
		await expect(
