...
async function appBase(page: Page): Promise<string> {
	...
	if (!resolved) {
		throw new Error(
			"OC.generateUrl unavailable — cannot resolve the humaniq router base.",
		);
	}
...
test.describe("hours process — booking, aggregation, approval lifecycle", () => {
	...
	test.beforeAll(async () => {
		...
		const res = await api.get(
			`${OR_BASE}/${REGISTER}/Timesheet?_limit=200`,
			{ headers: HEADERS },
		);
		expect(
			res.ok(),
			`Timesheet collection must be readable (${res.status()})`,
		).toBeTruthy();
		...
		const bySeed = (hours: number, status: string) =>
			rows.find(
				(r) =>
					Number(r.hours) === hours &&
					r.status === status &&
					r.period === "2026-05",
			);
		...
		expect(
			jansenTimesheetId,
			"seeded jansen 2026-05 submitted timesheet must exist",
		).toBeTruthy();
		expect(
			devriesTimesheetId,
			"seeded devries 2026-05 approved timesheet must exist",
		).toBeTruthy();
		expect(
			bakkerTimesheetId,
			"seeded bakker 2026-05 rejected timesheet must exist",
		).toBeTruthy();
		...
		expect(
			entry.ok(),
			`seed other-employee entry failed: ${entry.status()} ${await entry.text()}`,
		).toBeTruthy();
		const entryJson = await entry.json();
		otherTimesheetId = String(entryJson.timesheetId || "");
		expect(
			otherTimesheetId,
			"booking must auto-attach a 2026-06 timesheet",
		).toBeTruthy();
		cleanup.push({
			schema: "TimeEntry",
			id: entryJson?.["@self"]?.id || entryJson?.id,
		});
		...
		expect(
			submit.ok(),
			`submitting the other-employee timesheet failed: ${submit.status()} ${await submit.text()}`,
		).toBeTruthy();
		...
		expect(
			ts.ok(),
			`seed empty timesheet failed: ${ts.status()} ${await ts.text()}`,
		).toBeTruthy();
	...
	test.afterAll(async () => {
		...
		for (const { schema, id } of cleanup.reverse()) {
			if (!id) continue;
			await api
				.delete(`${OR_BASE}/${REGISTER}/${schema}/${id}`, {
					headers: HEADERS,
				})
				.catch(() => {});
		}
	...
	test("app shell opens at the timesheets list and the new hours pages are in the nav", async ({
		page,
	}) => {
		await page.goto("/index.php/apps/humaniq/", {
			waitUntil: "domcontentloaded",
		...
		await expect(page).toHaveURL(/\/apps\/humaniq\/timesheets$/, {
			timeout: 15_000,
		});
	...
	test("MijnUren create dialog shows exactly the six booking fields", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/uren");
		const addBtn = page
			.getByRole("button", { name: /Add Time entry/i })
			.first();
		await expect(
			addBtn,
			"MijnUren must offer Add (actionToggles.showAdd)",
		).toBeVisible({ timeout: 20_000 });
		await addBtn.click();
		...
		for (const label of [
			...
			await expect(
				...
				`field "${label}" must be on the form`,
			).toBeVisible({ timeout: 5_000 });
		}
		// Exactly six fields — not six-plus-something.
		await expect(
			dialog.locator(".cn-form-dialog__field"),
			"the form must render EXACTLY 6 fields",
		).toHaveCount(6);
		...
		for (const label of [
			...
			await expect(
				fieldLabel(label),
				`field "${label}" must NOT be on the form`,
			).toHaveCount(0);
		}
		...
		await dialog
			.getByRole("button", { name: /Annuleren|Cancel/i })
			.first()
			.click();
	...
	test("Mijn pages list only the signed-in user's records (fail-closed)", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/uren");
		...
		await expect(
			content,
			"jansen's seeded booking must render (userId admin)",
		).toContainText("Projectwerk project-alpha", { timeout: 20_000 });
		await expect(
			content,
			"devries' booking (userId null) must NOT leak",
		).not.toContainText("Supportdienst en onboarding");
		await expect(
			content,
			"bakker's booking (userId null) must NOT leak",
		).not.toContainText("gecorrigeerde boeking");

		await gotoRoute(page, "/mijn/urenstaten");
		await expect(
			content,
			"jansen's 152h timesheet must render",
		).toContainText("152", { timeout: 20_000 });
		await expect(
			content,
			"devries' 168h timesheet (userId null) must NOT leak",
		).not.toContainText("168");
		await expect(
			content,
			"bakker's 140h timesheet (userId null) must NOT leak",
	...
	test("MijnLoonstroken offers no authoring affordance", async ({ page }) => {
		...
		await expect(
			content,
			"the payslip list must render the seeded period",
		).toContainText("2026-05", { timeout: 20_000 });
		await expect(
			page.getByRole("button", { name: /Add Payslip/i }),
			"no Add button",
		).toHaveCount(0);
	});
	...
	test("booking through the MijnUren dialog creates a TimeEntry with derived hours", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/uren");
		await page
		...
		const fieldText = (
			await dialog.locator(".cn-form-dialog__field").allInnerTexts()
		).join("\n|\n");
		const startPos = fieldText.indexOf("Start");
		const endPos = fieldText.indexOf("End");
		expect(startPos, "the form must offer a Start field").toBeGreaterThan(
			-1,
		);
		expect(endPos, "the form must offer an End field").toBeGreaterThan(-1);
		expect(
			startPos,
			"Start must precede End on the booking form",
		).toBeLessThan(endPos);
		...
		await dialog
			...
			.nth(0)
			.fill("2026-07-15T12:00");
		await dialog
			.locator('input[type="datetime-local"]')
			.nth(1)
			.fill("2026-07-15T20:30");
		...
		await dialog
			.getByRole("button", { name: /^(Create|Save|Aanmaken|Opslaan)$/i })
			.first()
			.click();
		await expect(dialog, "create dialog must close on success").toBeHidden({
			timeout: 15_000,
		});
		...
		await expect(
			content,
			"the new booking must appear on MijnUren",
	...
	test("the auto-created timesheet lists the running total on MijnUrenstaten", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/urenstaten");
		const row = page
			.locator("#app-content tr, .app-content tr")
			.filter({ hasText: "2026-07" })
			.first();
		await expect(
			row,
			"the 2026-07 draft timesheet must be listed",
		).toBeVisible({ timeout: 20_000 });
		await expect(row, "hours must show the 8.00 aggregate").toContainText(
			"8",
		);
		...
		await expect(row, "status must show the draft label").toContainText(
			/Draft|Concept/,
		);
		await expect(row, "and never the raw stored code").not.toContainText(
			"draft",
		);
	});
	...
	test("submitting the draft timesheet stamps submittedAt", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/urenstaten");
		const row = page
			...
			.first();
		await row.click();
		await expect(page, "row click must open TimesheetDetail").toHaveURL(
			/\/timesheets\/[^/]+$/,
			{ timeout: 15_000 },
		);

		await expect(
			lifecycleButton(page, "submit"),
			"a draft timesheet offers Submit",
		).toBeVisible({ timeout: 20_000 });
		await lifecycleButton(page, "submit").click();

		await expect(
			dataCell(page, "Status"),
			"status must move to submitted",
		).toContainText(/Submitted|Ingediend/, { timeout: 20_000 });
		await expect(
			dataCell(page, "Submitted at"),
			"submittedAt must render non-empty (server-stamped)",
	...
	test("the approval queue lists the submitted timesheets", async ({
		page,
	}) => {
		...
		await expect(
			content,
			"the seeded jansen submitted 2026-05 row must be queued",
		).toContainText("2026-05", { timeout: 20_000 });
		await expect(
			content,
			"the other-employee 2026-06 row must be queued",
		).toContainText("2026-06");
		await expect(
			content,
			"the just-submitted 2026-07 row must be queued",
	...
	test("approving your own timesheet is refused by NoSelfApprovalGuard", async ({
		page,
	}) => {
		await gotoRoute(page, `/timesheets/${jansenTimesheetId}`);
		await expect(lifecycleButton(page, "approve")).toBeVisible({
			timeout: 20_000,
		});
		await lifecycleButton(page, "approve").click();
		await expect(
			page.getByTestId("cn-lifecycle-actions-error"),
			"the guard refusal must surface inline",
		).toContainText(/eigen urenstaat/i, { timeout: 20_000 });
		await expect(
			dataCell(page, "Status"),
			"the state must not change",
		).toContainText(/Submitted|Ingediend/);
	});
	...
	test("approving another employee's timesheet stamps approvedBy/approvedAt", async ({
		page,
	}) => {
		await gotoRoute(page, "/timesheets/approval");
		const row = page
		...
		).toContainText(/Approved|Goedgekeurd/, { timeout: 20_000 });
		await expect(
			dataCell(page, "Approved by"),
			"approvedBy must be the acting session uid, stamped on the carrying write",
		).toContainText("admin", { timeout: 15_000 });
		await expect(
			dataCell(page, "Approved at"),
			"approvedAt must render non-empty",
		).toContainText(/\d{4}/, { timeout: 15_000 });
	});
	...
	test("an empty draft timesheet cannot be submitted and offers no approve", async ({
		page,
	}) => {
		await gotoRoute(page, `/timesheets/${emptyTimesheetId}`);
		await expect(lifecycleButton(page, "submit")).toBeVisible({
			timeout: 20_000,
		});
		await expect(
			lifecycleButton(page, "approve"),
			"draft offers no approve transition",
		).toHaveCount(0);

		await lifecycleButton(page, "submit").click();
		await expect(
			page.getByTestId("cn-lifecycle-actions-error"),
			"the empty-timesheet refusal must surface with the Dutch guard message",
		).toContainText(/bevat geen urenboekingen|telt op tot nul uren/, {
			timeout: 20_000,
		});
		await expect(
			dataCell(page, "Status"),
			"the timesheet must remain draft",
		).toContainText(/Draft|Concept/);
	});

	// Scenario: An impossible time span is refused
	// (time-entry-capture — an end before start is refused with a structured
	// 422; the dialog stays open and nothing is persisted.)
	test("a booking whose end precedes its start is refused", async ({
		page,
	}) => {
		await gotoRoute(page, "/mijn/uren");
		await page
		...
		await dialog
			.locator('input[type="datetime-local"]')
			.nth(0)
			.fill("2026-07-16T17:00");
		await dialog
			.locator('input[type="datetime-local"]')
			.nth(1)
			.fill("2026-07-16T09:00");
		await dialog
			.locator('input[type="text"], textarea')
			.nth(0)
			.fill(`Onmogelijke boeking ${RUN_ID}`);
		await dialog
			.getByRole("button", { name: /^(Create|Save|Aanmaken|Opslaan)$/i })
			.first()
			.click();
		...
		await expect(
			dialog,
			"the dialog must not close on a refused write",
		...
		await expect(
			page.locator("#app-content, .app-content").first(),
			"the refused booking must not appear",
	...
	test("an approved timesheet's entries show no edit affordance", async ({
		page,
	}) => {
		...
		await expect(
			bookings,
			"the Urenboekingen list must render the seeded entries",
		).toContainText("Supportdienst en onboarding", { timeout: 20_000 });
		await expect(
			bookings.locator(".cn-object-list-widget__add"),
			"no add affordance on the bookings list",
		).toHaveCount(0);
		await expect(
			bookings.getByRole("button", {
				name: /edit|bewerk|delete|verwijder/i,
			}),
	...
	test("a rejected timesheet re-submits and the stamping clears the approval fields", async ({
		page,
	}) => {
		await gotoRoute(page, `/timesheets/${bakkerTimesheetId}`);
		await expect(dataCell(page, "Status")).toContainText(
			/Rejected|Afgekeurd/,
			{ timeout: 20_000 },
		);
		await expect(
			dataCell(page, "Rejection reason"),
			"the seeded reason must render read-only",
		).toContainText("komt niet overeen", { timeout: 15_000 });

		await expect(
			lifecycleButton(page, "submit"),
			"rejected offers Submit",
		).toBeVisible({ timeout: 15_000 });
		await lifecycleButton(page, "submit").click();
		...
		await expect(
			dataCell(page, "Rejection reason"),
			"the stamping must clear the previous reason",
