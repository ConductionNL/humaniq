...
async function setLanguage(
	...
	expect(
		response.ok(),
		`switching the session language to ${lang} must succeed`,
	).toBeTruthy();
}
...
function addButton (page: Page) {
  return page.locator('[data-testid="cn-cta-primary"]')
}

/** The open form dialog. */
function dialog (page: Page) {
  return page.locator(
		'[data-testid-modal="cn-form-dialog"][data-testid-phase="form"]',
	)
}

/** One auto-generated field, addressed by its schema property key. */
function fieldFor (page: Page, key: string) {
  return dialog(page).locator(`[data-cn-field="${key}"]`)
}

/** The options of an open NcSelect dropdown. */
function dropdownOptions (page: Page) {
  return page.locator('.vs__dropdown-menu li, [role="option"]')
}
...
test.afterAll(async () => {
	...
	const listed = await api.get(`${OR_OBJECTS}/Vacancy?_limit=100`, {
		headers: OCS_HEADERS,
	});
	if (listed.ok()) {
		...
		for (const row of rows) {
			if (typeof row.title === "string" && row.title.includes(RUN_ID)) {
				await api.delete(
					`${OR_OBJECTS}/Vacancy/${row["@self"]?.id ?? row.id}`,
					{ headers: OCS_HEADERS },
				);
			}
		}
...
test.describe("i18n form surface — a form reads in the session language", () => {
	...
	test("a Dutch session reads Dutch field labels", async ({ page }) => {
		...
		await expect(fieldFor(page, "breakMinutes")).toContainText(
			/Pauze \(minuten\)/i,
		);
		await expect(fieldFor(page, "description")).toContainText(
			/Omschrijving/i,
		);
		...
		await expect(fieldFor(page, "breakMinutes")).not.toContainText(
			"Break (minutes)",
		);
		await expect(fieldFor(page, "description")).not.toContainText(
			"Description",
		);
	});
	...
	test("an English session reads the English source strings", async ({
		page,
	}) => {
	...
	test("an English session reads the declared label, never the Dutch stored code", async ({
		page,
	}) => {
		...
		expect(
			optionText,
			"the English session offers the declared English labels",
		).toMatch(/Published/);
		expect(optionText, "and never the raw Dutch code").not.toMatch(
			/gepubliceerd/i,
		);
	});
	...
	test("a Dutch session reads Dutch status options", async ({ page }) => {
		...
		expect(optionText, "the Dutch session offers Dutch labels").toMatch(
			/Concept/i,
		);
		expect(
			optionText,
			"and the English label is not what renders",
		).not.toMatch(/\bDraft\b/);
	});

	// Scenario: Selecting a translated option stores the raw value
	// @e2e humaniq-i18n-form-surface::selecting-a-translated-option-stores-the-raw-value
	test("picking a Dutch-labelled option persists the raw schema code", async ({
		page,
	}) => {
		await setLanguage(api, "nl");
		await openCreateForm(page, "/vacancies");

		await fieldFor(page, "title")
			.locator("input")
			.fill(`${RUN_ID} vacature`);
		await fieldFor(page, "status").click();
		await expect(dropdownOptions(page).first()).toBeVisible({
			timeout: 10_000,
		});
		...
		await page
			.getByRole("button", { name: /Aanmaken|Create|Opslaan|Save/i })
			.first()
			.click();
		...
		const created = rows.find(
			(r: Record<string, unknown>) =>
				typeof r.title === "string" && r.title.includes(RUN_ID),
		);

		expect(created, "the form actually created the object").toBeTruthy();
		expect(
			created.status,
			"the stored value is the raw schema code, not the Dutch label",
		).toBe("concept");
	});
});
