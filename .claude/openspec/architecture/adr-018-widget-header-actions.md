...
**See also**: [ADR-036](adr-036-universal-widget-manifest.md) formalises `header-actions` as a canonical named slot in the v2 widget manifest. The slot contract defined here is unchanged; v2 only standardises how widgets are placed into the slot via the unified `widgets[]` array with `slot: "header-actions"`.
