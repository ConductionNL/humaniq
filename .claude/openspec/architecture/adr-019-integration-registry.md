...
**See also**: [ADR-036](adr-036-universal-widget-manifest.md) expresses integration widgets as `kind: "widget"` entries in the unified component registry. The integration contract defined here (registry shape, provider interface, surface contracts) is unchanged; v2 only standardises how integration widgets are placed in manifest pages via the unified `widgets[]` array.
