...
### Mount-survivable bootstrap

Every consumer's `main.js` MUST shallow-clone the lib's `defaultPageTypes`
and `customComponents` before passing them to `CnAppRoot`, AND MUST wrap
the `loadTranslations` call fire-and-forget so a 404 from the i18n endpoint
can't block `app.$mount`. Both rules emerged from the same fleet-wide
breakage class: page mount silently fails in production-shaped containers
when either side throws.

- **Shallow-clone `defaultPageTypes` and `customComponents`** before
  passing them as props to `CnAppRoot`. Vue 2 + Rollup's `Object.freeze`
  on the namespace re-exports causes `_Ctor` lookup failures inside
  `Vue.extend` when the renderer mutates the registry to add the
  page-instance ctor:
  ...
  Pass the cloned props (`pageTypesProp`, `customComponentsProp`) to
  `CnAppRoot`. The clone works on every beta — even after lib
  `1.0.0-beta.12` patched the rollup namespace freezes upstream, the
  pattern stays as a defensive belt-and-braces.

- **Wrap `loadTranslations` fire-and-forget** so a missing
  `/custom_apps/<app>/l10n/<locale>.json` (404 in dev containers, or
  silent cache-busting on first deploy) can't block the Vue mount:

  ```js
  import { loadTranslations } from '@nextcloud/l10n'

  function tryLoadTranslations () {
    loadTranslations(appId, locale).catch(() => {})
  }
  tryLoadTranslations()
  ...
  Mounting unconditionally means a missing locale file degrades to
  English keys, not a blank page. Mirror `en_US.json` from `en.json` to
  silence the locale fetch entirely on production deploys.

- See `decidesk/src/main.js` (commits `50e4df7c` for `loadTranslations`,
  `866ff132` for the shallow-clone) as the canonical reference. The
  same pattern is in every fleet app's `main.js` after the beta.13 wave.

- Cross-references: ADR-024 (App manifest) for the renderer contract,
  ADR-026 (Store migration patterns) for the corresponding store
  bootstrap.

### Dependencies & app availability

Every fleet app depends on at least one peer Nextcloud app (OpenRegister,
typically) to function. To avoid each consumer re-rolling its own
"OpenRegister is required" empty-state, `CnAppRoot` ships an
**app-availability guard** built in.

- **Default behaviour**: `CnAppRoot` defaults `requiresApps:
  ['openregister']`. On mount it calls `getCapabilities()` from
  `@nextcloud/capabilities` (peer dep) and compares against the listed
  app ids. When any required app is missing, `CnAppRoot` renders an
  `<NcEmptyContent>` empty-state linking to the OpenRegister
  app-store integration page instead of mounting the renderer.

- **Opt-out**: consumer apps that don't need OpenRegister (e.g.
  **mydash**, the docs/styleguide app, **nldesign**) MUST pass
  `:requires-apps="[]"` on `<CnAppRoot>`. The empty array bypasses
  the guard entirely; the renderer mounts unconditionally.

  ```vue
  <CnAppRoot :manifest="manifest" app-id="mydash" :requires-apps="[]">
    <!-- ... -->
  </CnAppRoot>
  ```

- **Custom empty-state**: apps that want a branded missing-app screen
  override the `or-missing` scoped slot. The slot receives
  `{ missingApps }` (an array of unresolved app ids) so consumers can
  render per-app messaging:

  ```vue
  <CnAppRoot :manifest="manifest" app-id="myapp">
    <template #or-missing="{ missingApps }">
      <MyBrandedDependencyScreen :missing="missingApps" />
    </template>
  </CnAppRoot>
  ```

- **Network-failure fallback**: when `getCapabilities()` rejects (admin
  restricted, offline, unexpected response shape), `CnAppRoot` logs the
  error and falls through to rendering the manifest. The data layer
  surfaces the actual problem on first request rather than masking it
  with a misleading empty-state. Don't add a second guard upstream.

- **Loading state**: while the capabilities check is in flight,
  `CnAppRoot` renders a thin `<NcLoadingIcon>` so consumers don't see a
  flash of "renderer-then-empty-state" on slow connections.

- **Hand-rolled wrappers MUST go.** Apps that previously wrapped
  `CnAppRoot` in their own `App.vue` to check OpenRegister availability
  (e.g. `procest`'s legacy `App.vue` with `hasOpenRegisters` /
  `storesReady` watchers) MUST drop the wrapper and rely on the lib
  default. Pure-manifest parity with `decidesk`/`pipelinq`/`softwarecatalog`
  is the goal — consumer-side `App.vue` files only justify their
  existence by holding global modal hosts, dialog hosts, or sidebar
  wiring that `CnAppRoot` doesn't already provide.

- **Fleet status (2026-05-10)**: `decidesk`, `pipelinq`, `softwarecatalog`,
  `zaakafhandelapp` use the lib default. `procest` migrating in a
  follow-up PR (App.vue guard removed). `mydash` MUST opt out
  (`:requires-apps="[]"`) per ADR-022 (no OR install-time dependency).

- **Source**: `@conduction/nextcloud-vue` `CnAppRoot` PR
  [#191](https://github.com/ConductionNL/nextcloud-vue/pull/191) and
  the openspec change `cnapproot-app-availability-guard` (REQ-OR-1
  through REQ-OR-7) for the normative contract. Lib version floor:
  `1.0.0-beta.27` or later.

- Cross-references: ADR-022 (Apps consume OR abstractions) for the
  OR-everywhere baseline; ADR-024 (App manifest) for the renderer
  contract.
