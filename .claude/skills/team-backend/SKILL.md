...
#### MCP Tool Wiring (ADR-035)

If the proposal's `### MCP coverage` section declares `Adds tool: {appId}.{toolName}` (or `Extends existing tool: ...`), the implementation MUST land all three pieces in the same PR:

1. **Tool descriptor in `listTools()`** — add the `{appId}.{toolName}` entry to the app's `OCA\{AppNamespace}\Mcp\{AppName}ToolProvider::listTools()` return. Descriptor MUST include a non-empty `description` and a JSON-Schema `parameters` object (even if `parameters` is `{type: 'object', properties: {}, additionalProperties: false}` for tools that take no arguments).
2. **`invokeTool()` branch** — call the existing service / controller / command method that backs the user-callable action. Per [ADR-034](../../../openspec/architecture/adr-034-ai-chat-companion.md) Decision 7 the tool MUST run with the current session user's permissions; do not impersonate, do not elevate, do not substitute a service account.
3. **Unit test of the tool descriptor** — instantiate the provider with the same DI shape `lib/AppInfo/Application.php` registers, call `listTools()`, and assert (a) the returned array contains the new tool id, (b) the id is exactly `{appId}.{toolName}` (no other prefix), (c) the descriptor has a non-empty description and a valid JSON-Schema `parameters` object.

If the app has no `IMcpToolProvider` yet (first tool for that app), create one under `lib/Mcp/{AppName}ToolProvider.php` and register it in `lib/AppInfo/Application.php` under the DI alias `OCA\OpenRegister\Mcp\IMcpToolProvider::{appId}`. Reference implementation: `decidesk/lib/Mcp/DecideskToolProvider.php`.

If the proposal declares `No MCP surface — {reason}`, do nothing on the MCP side. The reviewer's soft-flag heuristic may still fire on the PR and emit an advisory — that is informational, not blocking.

Reference: [ADR-035 MCP per-app coverage](../../../openspec/architecture/adr-035-mcp-per-app-coverage.md).
