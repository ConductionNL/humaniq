...
**MCP Coverage Check (ADR-035)**

Every proposal MUST contain a `### MCP coverage` section (under `## Impact`, or under `## What Changes` when no `## Impact` exists). REJECT the proposal at Step 2 — without proceeding to team-architect — if any of the following hold:

- [ ] The `### MCP coverage` section is missing entirely.
- [ ] The section contains only a bare `n/a`, `not applicable`, `none`, or equivalent boilerplate.
- [ ] The `No MCP surface — ...` answer omits the one-sentence reason.
- [ ] The `Adds tool: ...` answer omits the `{appId}.{toolName}` identifier or uses a name that does not follow the `{appId}.{toolName}` namespacing rule (mechanically enforced by OR's `McpToolsService` per ADR-034 Decision 3).

The three valid answer forms (one per declared tool when `Adds tool` is repeated):
- `Adds tool: {appId}.{toolName}`
- `Extends existing tool: {appId}.{toolName}` — body explains wire compatibility in one or two sentences.
- `No MCP surface — {one-sentence reason}`

Reference: [ADR-035 MCP per-app coverage](../../../openspec/architecture/adr-035-mcp-per-app-coverage.md). Cite this requirement when rejecting.
