...
## ADR Filename Convention (READ BEFORE CREATING ANY ADR FILE)

ADR files in `{app}/openspec/architecture/` MUST be named `adr-NNN-slug.md` with **exactly one** `adr-` prefix. Before writing the filename:

1. Strip any leading `adr-` from the slug (loop until gone — handles stutter input like `adr-adr-022-foo`).
2. Compose: `adr-{normalised-slug}.md`.
3. If the target file already exists, **refuse to overwrite** — surface the conflict to the user with the existing file's content snippet so they can decide between updating, renaming, or aborting.

This convention is enforced by `scripts/run-hydra-gates.sh` (gate-15 dashboard-antipattern lives near the same checker family). Pipelinq#525 and decidesk's three stutter files were the historical evidence — the upstream cause was a missing prefix-strip in `concurrentie-analyse/scripts/push_spec_pipeline.py`, fixed 2026-05-23.
...
### Step 6b: MCP Coverage Review (ADR-035)

team-po confirmed the proposal contains a well-formed `### MCP coverage` section before this skill ran. Your job at Step 6b is to confirm the chosen answer is *appropriate* for the change's actual scope:

- [ ] If the proposal says `Adds tool: {appId}.{toolName}`: does `tasks.md` include the corresponding `IMcpToolProvider` edit and a unit test of the tool descriptor? If yes, accept. If the tool is declared but the tasks omit the wiring, REQUEST CHANGES — the builder will not infer the wiring from `proposal.md` alone.
- [ ] If the proposal says `Extends existing tool: {appId}.{toolName}`: is the wire-compatibility approach (e.g. new optional parameter, additive response field) credible given the change? If the extension would break existing tool callers, REQUEST CHANGES.
- [ ] If the proposal says `No MCP surface — {reason}`: is the stated reason consistent with the actual diff scope in `tasks.md`? Example acceptable: a schema-only register-template patch in a `kind: config` change. Example unacceptable: a `kind: code` change that adds a new controller action and a new index/detail view but claims "No MCP surface — internal refactor". If inconsistent, REQUEST CHANGES.
- [ ] If the change touches an app that has no `IMcpToolProvider` at all, AND the change is the first user-actionable surface for that app, prefer `Adds tool` over `No MCP surface` unless the proposer has explicitly documented an opt-out in the app's `openspec/project.md` `## MCP coverage` block.

Reference: [ADR-035 MCP per-app coverage](../../../openspec/architecture/adr-035-mcp-per-app-coverage.md). This step does NOT read the per-app `IMcpToolProvider` source code — only the proposal and tasks. Source-code consistency is the builder's responsibility.
