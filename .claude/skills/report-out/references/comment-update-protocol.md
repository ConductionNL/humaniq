...
1. Read the existing body — note the date heading and section structure. The same user can use different header conventions across tracking issues (`## Status update — DATE` vs `### DD-MM-YYYY` vs `**DD-MM-YYYY**`). Do not assume a single house style — always read at least one prior comment on this specific issue and match its convention.
