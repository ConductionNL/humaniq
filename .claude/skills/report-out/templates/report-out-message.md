...
```
Report out:
{2–4 high-level summary lines describing the day's themes — NOT commit titles}
...
- **No newline** between the heading and the first line.
- **No day name or date** in the heading — Slack already shows the timestamp.
- **No leading `- ` dashes** on any line — summary lines, PR's: list, and Issues: list are all plain lines. The user applies Slack's native bullet formatting after pasting; Markdown dashes do not render as bullets in Slack and just appear as literal `-` characters.
...
- **PR's: / Issues: lists below MUST use full URLs** — bare `repo#N` text does NOT auto-link in Slack, so the canonical clickable handle for every referenced item lives in these lists.
- **Summary bullets at the top MAY use `repo#N` shorthand IF the same item also appears as a full URL in the lists below** — readers can click the list entry. Items referenced in bullets that are NOT in the lists below must be full URLs (branch links, third-party items, in-body PR refs that aren't in the user's own activity).

**Line style — keep it scannable:**
...
- One outcome per line. The user adds bullet formatting in Slack — the skill emits plain lines only.

**Reviews summary line (required when reviews exist):**
- When `{REVIEWED_PRS_TODAY}` is non-empty, the final summary section MUST include one consolidated line of the form: `{N} PR reviews uitgevoerd verspreid over {comma-separated unique repo names}`.
- Do NOT include verdict breakdowns (no `(N CHANGES_REQUESTED, M APPROVED)` tallies). Outcomes only.
- Do NOT list each reviewed PR by URL — keep the line single and roll the repo names up.
- This line is in addition to the 2–4 high-level summary lines, not counted against the max-6 limit.
...
8 PR reviews uitgevoerd verspreid over {repo-d}, {repo-e}, {repo-f}
...
- **No newline** between the heading and the first line.
- **No leading `- ` dashes** on summary lines, PR's: list, or Issues: list. The user applies Slack's bullet formatting after pasting; Markdown dashes appear literally as `-` and don't render as bullets.
- **Only include PRs the GitHub user authored or merged today.** PRs by team members in the same repos do not belong here.
- Lists (PR's:, Issues:) MUST contain bare full URLs (`https://github.com/...`). Do NOT wrap them in Slack's `<URL|label>` syntax — that syntax does not render via copy-paste from chat into Slack composer.
- Summary lines MAY use `repo#N` shorthand for items already linked in the lists below; otherwise use bare full URLs.
- Combine related commits into one line — never one line per commit.
...
- Maximum 6 high-level summary lines. If a day has more, combine ruthlessly.
