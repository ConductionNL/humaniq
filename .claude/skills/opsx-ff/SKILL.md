...
- **tasks.md MUST NOT exceed 20 unindented `- [ ]` lines** — the Hydra supervisor rejects specs with more (`grep -c '^- \[ \]' tasks.md > 20` triggers `needs-input`). Before writing tasks.md, count the planned checkboxes: each `- [ ]` at column 0 costs 1. **Acceptance criteria MUST be plain text bullets** (`- criterion text`, NOT `- [ ] criterion text`) — using checkboxes for criteria quickly blows past the cap. Quality checklist items (tests, docs, i18n) should also be plain text reminders, not checkboxes. If the task count genuinely requires more than 20 checkboxes, split the change per ADR-032.
