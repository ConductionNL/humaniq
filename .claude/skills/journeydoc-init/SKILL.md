...
As of 2026-05-13, `ConductionNL/nextcloud-app-template` (PR #36) ships
a default Docusaurus site + journeydoc scaffold by default, so every
**newly-scaffolded** Conduction app already has `docs/` + the
tutorials skeleton + `tests/e2e/docs-screenshots.spec.ts` +
`playwright.config.ts` + `tests/e2e/global-setup.ts`. This skill is
therefore a **brown-field operation by default** — its job is to
audit + migrate + extend, not to bootstrap from scratch. The only
true green-field case left is a *legacy* app whose `development`
branch predates the template's docs scaffold.

Check what's there:

- `docs/` exists with a Docusaurus config (using
  `@conduction/docusaurus-preset` `createConfig`) → **brown-field**,
  the common path: apply the Step-4.5 migrations, then add/extend
  tutorials per Step 6.
- `docs/` exists but ONLY as plain markdown (no `docusaurus.config.js`)
  → **legacy green-field**: stand up the site first by copying the
  template's `docs/` structure (see `nextcloud-app-template`'s `docs/`
  on its default branch) and adapting per Step 4.5d (domain swap), then
  proceed.
- `docs/` doesn't exist at all → very old app. Adopt the template's
  `docs/` wholesale (same as legacy-green-field above) before
  continuing.

Use AskUserQuestion only if the situation is ambiguous (e.g. there's
both an old `docusaurus/` dir and a new `docs/`).
...
| `global-setup.ts.template` | `{APP_DIR}/tests/e2e/global-setup.ts` |
...
- `playwright.config.snippet.ts` → `{APP_DIR}/playwright.config.ts`
  (this snippet wires `globalSetup` + `use.storageState` on both the
  regression `chromium` project and the `docs-capture` project — the
  global-setup file landed in the row above is what it points at)
- `package.json.snippet.json` → `{APP_DIR}/package.json` (adds
  `@playwright/test ^1.49.0` to `devDependencies` + the
  `test:e2e` / `test:e2e:docs` / `test:e2e:install` npm scripts; CI
  still runs `npm ci --legacy-peer-deps && npx playwright install
  chromium && npm run test:e2e:docs` once these are spliced)
...
- `{{GOAL}}` / `{{PREREQS}}` / `{{STEPS}}` / `{{VERIFICATION}}` /
  `{{COMMON_ISSUES}}` / `{{REFERENCE}}` — left as **HTML comments**
  per the template (`<!-- TODO: {{GOAL}} -->` etc.). They MUST be
  HTML comments, NOT bare `{{TODO: …}}` text — Docusaurus 3 parses
  `.md` as MDX so bare `{…}` is read as a JavaScript expression and
  `npm run build` fails with "Could not parse expression with acorn"
  (this broke procest's and openbuilt's docs builds in May 2026 and
  was fixed in `hydra#270`). The template is already HTML-comment-
  wrapped — don't change it back. The prose is for the human or for
...
`test('UN <slug>', async ({ page }) => { … })` block (user track) or
`test('AN <slug>', async ({ page }) => { … })` (admin track) matching
the markdown filename. The block includes a `// TODO` comment
pointing the author at `journeydoc-add-story` for filling in the
actual flow.

The template (`docs-screenshots.spec.ts.template` in
`templates/journeydoc/`) ships three helpers that every filled spec
should use — keep them at the top of the generated file:

- `shoot(page, path)` — `await page.screenshot({ path })` with the
  conventional `docs/static/screenshots/tutorials/<track>/<file>.png`
  output path resolved from the `path` argument.
- `dismissOverlays(page)` — closes Nextcloud's `firstrunwizard`
  modal if it appears (otherwise it intercepts pointer events and the
  first run times out). Call once at the start of each test, before
  any clicks.
- `go(page, route)` — wrapper around `page.goto(BASE_URL + route)`
  using the global-setup-installed admin session.
...
Next:
  1. Fill the HTML-comment TODOs in `docs/tutorials/{user,admin}/*.md`.
     Use `/journeydoc-add-story <track> "<title>"` for incremental
     additions.
  2. Run `/journeydoc-instrument <vue-file>` on each capture-relevant
     app-specific component (modals, primary buttons, list-row actions,
     context menus). Most chrome is already instrumented in
     @conduction/nextcloud-vue's Cn* components (cn-app-root, cn-nav,
     cn-page-header, cn-object-list, cn-object-row, cn-modal, etc.)
     so per-app testids only matter for app-specific surfaces.
  3. Install Playwright deps + browsers, then run the capture spec:
       npm ci --legacy-peer-deps
       npx playwright install chromium
       npm run test:e2e:docs        # alias for: playwright test --project docs-capture
     The capture spec will use the spliced `tests/e2e/global-setup.ts`
     to log into Nextcloud once and reuse the session.
  4. DNS is already handled: *.conduction.nl is a wildcard CNAME →
     conductionnl.github.io. The only post-merge ops step is to
     enable GitHub Pages on the repo if it isn't already:
       gh api -X POST repos/ConductionNL/{APP_SLUG}/pages \
         --input - <<<'{"source":{"branch":"gh-pages","path":"/"}}'
     (GitHub will auto-provision the cert from the CNAME file shipped
     in `docs/static/CNAME`.)
