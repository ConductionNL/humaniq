...
|---|---|---|
| `--trigger-label <label>` | Add a Hydra trigger label to every created issue so Hydra picks up the work automatically. Omit to create issues without queuing them. | `--trigger-label wilco-ready-to-build` |

If `--trigger-label` is not provided, check whether `~/hydra/secrets/.env` (or `$HYDRA_ROOT/secrets/.env`) contains `HYDRA_LABEL_PREFIX`. If it does, suggest the constructed label (`{prefix}-ready-to-build`) to the user in the Step 3.5 preview — but do **not** apply it unless they confirm.
...
Parse any flags from the user's invocation before doing anything else:
- `--trigger-label <label>` → store as `{TRIGGER_LABEL}` (e.g. `wilco-ready-to-build`)

If no `--trigger-label` was given, check for `HYDRA_LABEL_PREFIX` in `~/hydra/secrets/.env`:
```bash
grep '^HYDRA_LABEL_PREFIX=' ~/hydra/secrets/.env 2>/dev/null | cut -d= -f2
```
If a prefix is found, set `{TRIGGER_LABEL_SUGGESTION}` = `{prefix}-ready-to-build` for use in the preview step.
...
**Determine the base branch** (what feature branches should be cut from / PR'd back to). Priority:
1. `gh repo view <owner/repo> --json defaultBranchRef --jq '.defaultBranchRef.name'` — the repo's default branch is authoritative.
2. If the default is `main` but a `development` branch also exists (`gh api repos/<owner>/<repo>/branches/development --silent 2>/dev/null && echo yes`), prefer `development` — Hydra's convention is `development` → PR → `development` (see CLAUDE.md "Git Workflow").
3. The user can override the chosen base branch in the Step 3.5 preview.

Store as `{BASE_BRANCH}`. The **feature branch name** follows Hydra's convention `feature/<issue-number>/<change-name>` and is resolved in Step 4 once the issue number is known.
...
```json
{
  ...
  "repo": "<owner/repo>",
  "base_branch": "<BASE_BRANCH from Step 2>",
  "feature_branch": null,
  "created": "<ISO-date>",
  "tracking_issue": null,
  "tasks": [
...
Labels: openspec, <app-name>, <spec-1>, <spec-2>, ...[, <trigger-label> 🚀]
Base branch: <BASE_BRANCH>
Feature branch (resolved after issue is created): feature/<N>/<change-name>
...
Base branch: <BASE_BRANCH> (shared)

1. 📌 [OpenSpec] [<app-name>] <change-1>
   Labels: openspec, <app-name>, <spec-a>, <spec-b>[, <trigger-label> 🚀]
   Feature branch: feature/<N>/<change-1>
   Tasks: X checkboxes

2. 📌 [OpenSpec] [<app-name>] <change-2>
   Labels: openspec, <app-name>, <spec-c>[, <trigger-label> 🚀]
   Feature branch: feature/<M>/<change-2>
   Tasks: Y checkboxes

...
```

Include the trigger label in the preview if `{TRIGGER_LABEL}` is set. If only `{TRIGGER_LABEL_SUGGESTION}` is set (no explicit flag), add a note: "ℹ️ `{TRIGGER_LABEL_SUGGESTION}` detected from secrets/.env — add `--trigger-label {TRIGGER_LABEL_SUGGESTION}` to queue for Hydra." Do NOT add it automatically.
...
- `{TRIGGER_LABEL}` — if set, append to every issue's label list

**Trigger label color**: use `0075CA` (blue, same as `ready-to-build`) when creating a missing trigger label.

Ensure all required labels exist (create if missing) — do this **once** before the loop, collecting all unique labels across all changes (including the trigger label if set):
...
- The body contains a summary from `proposal.md`, followed by a `## Branch` section (see below), then a `## Tasks` section with task checkboxes grouped under section headers from `tasks.md`
...
**Two-step body write (required for the `## Branch` section):**

The feature branch name embeds the issue number, which isn't known until the issue exists. So:

1. Create the issue with a placeholder `## Branch` section: `Base: <BASE_BRANCH> · Feature branch: _resolving…_`.
2. Capture the returned `<N>` (issue number).
3. Compute `feature_branch = "feature/<N>/<change-name>"`.
4. Edit the issue body, replacing the placeholder with the resolved `## Branch` section below — `gh issue edit <N> --repo <owner/repo> --body "<updated body>"` or MCP `update_issue`.

**Issue body structure** (outer fence uses `~~~` so the inner ` ```bash ` block renders correctly):
...
**Work on this branch — do NOT start work on `<BASE_BRANCH>` and do NOT create a different branch.**

- Base branch: `<BASE_BRANCH>`
- Feature branch: `feature/<N>/<change-name>`

Before starting:

```bash
git fetch origin
# If the branch already exists remotely, check it out:
git checkout feature/<N>/<change-name> 2>/dev/null || \
  git checkout -b feature/<N>/<change-name> origin/<BASE_BRANCH>
```

Open the PR against `<BASE_BRANCH>` when ready.
...
**For each change**, update its `plan.json` with:
- `tracking_issue` — the created issue number
- `base_branch` — the `{BASE_BRANCH}` resolved in Step 2
- `feature_branch` — `feature/<N>/<change-name>` (same value written into the issue body's `## Branch` section)

Save at `openspec/changes/<change-name>/plan.json` in the project directory. Downstream tools (`/opsx-apply`, the Hydra builder) read these fields to check out the correct branch instead of branching off the default or `main`.
