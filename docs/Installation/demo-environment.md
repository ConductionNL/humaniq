...
This page gets a working Humaniq running on your own machine in two commands. You end with HR records and payroll pro-formas.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/humaniq/development/humaniq-compose.yaml
```
...
==> installing humaniq <version>
==> apps present: humaniq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8618/status.php
```
...
| What | Where |
| --- | --- |
| **Humaniq** | [http://localhost:8618/apps/humaniq/](http://localhost:8618/apps/humaniq/) |
| Admin interface | [http://localhost:8618](http://localhost:8618) — `admin` / `admin` |
...
| `humaniq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing humaniq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8618/apps/humaniq/"
...
curl -s -u admin:admin "http://localhost:8618/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f humaniq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f humaniq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f humaniq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
