...
- [avg-dsr](../../changes/archive/2026-07-15-avg-dsr/) _(archived 2026-07-15)_
  — AVG (GDPR) data-subject-rights orchestration over OpenRegister's `DsarService`: a `DsrRequest`
  lifecycle record, the subject-mapping and two-way export rendering for Art 15 inzage / Art 20
  portabiliteit, the explicit privileged-session establishment `DsarService::assertPrivileged()`
  requires, the retention-guarded two-path erase for Art 17 vergetelheid, the Art 16 rectificatie
  pass-through, and the guarded `occ`/HTTP invocation surfaces.

## Purpose

`OCA\OpenRegister\Service\DsarService` already implements the heavy lifting for AVG data-subject
rights: locating every object referencing a subject (Art 15 inzage / Art 20 portabiliteit, the same
call rendered two ways), soft-deleting every object matching a subject in one wholesale sweep (Art
17 vergetelheid), and updating a single object with DSAR attribution (Art 16 rectificatie). hrmq
owns no database tables and no PII-detection/entity-matching logic of its own (ADR-022); this
capability adds a thin hrmq orchestration layer over those three methods — never a reimplementation
of anonymisation, entity matching, or soft-delete.

Every `DsarService` public method requires an explicitly-privileged (admin) Nextcloud session and
throws `RuntimeException` otherwise — there is no degrade path. hrmq's `occ` commands run in plain
CLI with no ambient request and therefore no session by default; this capability designs the
session establishment explicitly for both the CLI and the HTTP surface (`AvgDsrService`,
`PrivilegedSessionResolver`, `AvgDsrController`).

The second load-bearing correctness requirement: hrmq's payroll/loonadministratie data carries a
statutory 7-year fiscal retention duty (AWR art. 52 lid 4). `DsarService::eraseObjectsForSubject()`
has no per-object exclusion parameter — it is a single wholesale sweep over every object matching a
subject. Calling it unguarded on an employee who has payslips inside their 7-year retention window
would violate AWR art. 52 lid 4 the moment it ran. This capability implements that partiality
itself (`AvgDsrRetentionClassifier`, the two-path erase in `AvgDsrService`), since `DsarService`
cannot: a retention-locked object is structurally never passed into `eraseObjectsForSubject()` or
`rectifyObjectForSubject()`, and is always reported, never silently skipped.
...
- **GIVEN** `AvgDsrController`'s admin gate somehow passed for a caller `DsarService::assertPrivileged()` would still reject
