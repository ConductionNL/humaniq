...
Declarative register foundation for humaniq's Wave-1 contribution to portaliq,
...
### Requirement: humaniq's portal-facing register surface is declarative and UUID-scoped (REQ-PSCH-000)
...
- @e2e exclude declarative register configuration with no humaniq UI surface — verified mechanically by the JSON gates and the gate-28 helper; the rendering surface is portaliq's
...
- @e2e exclude declarative register configuration with no humaniq UI surface this change — verified mechanically by python3 json.load + the gate-28 helper (check_schema_property_meta.py), and consumed by portaliq, not by any humaniq page
...
`requires: OCA\Humaniq\Lifecycle\NoSelfApprovalGuard` (separation of duties:
...
- AND both `approve` and `reject` require `OCA\Humaniq\Lifecycle\NoSelfApprovalGuard`
- @e2e exclude declarative lifecycle configuration enforced by OpenRegister's state machine, not by humaniq UI — the guard itself is already covered by the humaniq-timesheet-approval capability; shape verified mechanically via python3 json inspection
...
- @e2e exclude declarative register configuration with no humaniq UI surface this change — verified mechanically by python3 json.load + the gate-28 helper; the client-facing read over this field renders inside portaliq

### Requirement: Version bumps gate the re-import (REQ-PSCH-004)

The change MUST bump `info.version` in `lib/Settings/humaniq_register.json`
...
- @e2e exclude version metadata in declarative register configuration — verified mechanically via python3 json inspection and git diff; import gating is OpenRegister behaviour, not humaniq UI
