---
capability: payroll-mutation-reports
status: done
built_by: openspec/changes/archive/2026-07-14-payroll-mutation-reports
---
...
- [payroll-mutation-reports](../../changes/archive/2026-07-14-payroll-mutation-reports/) _(archived
  ...
  (`PayrollMutationService`): per-employee entered/left/changed/unchanged classification +
  ...
  PayrollRun/Payslip, so it cannot drift from the engine.

## Purpose

The one thing a loonadministrateur does before flipping a run from `draft` to `approved`: compare it
to the previous period — who joined, who left, whose gross/net/premies moved and by how much, and
what the total wage-cost delta is. This change produces that comparison deterministically as a pure
subtraction over the two runs' persisted payslips (meaningful now that `payroll-core-engine` stamps
cents-exact component fields), never a recomputation.
