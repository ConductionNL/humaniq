...
- [hrmq-personal-dashboard](../../changes/archive/2026-08-22-hrmq-personal-dashboard/) _(archived 2026-08-22, merged as humaniq#133, follow-up humaniq#137)_ — adds REQ-ACCR-006: the job stamps the denormalized `userId` on both the create and the monthly update path, so pre-existing `LeaveBalance` rows self-heal on the next accrual run rather than needing a repair step; accrual math, idempotency and the operator toggle are unchanged (kind: code)
