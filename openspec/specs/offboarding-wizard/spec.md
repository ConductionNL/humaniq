...
status: done
built_by: openspec/changes/archive/2026-07-13-offboarding-wizard-mvp
---
...
(BW 7:656, advisory), and the consistency of `Employee.endDate` with the
case's last working day (BW 7:667). MVP-scoped from the 2026-05
`spec/offboarding-wizard` draft's 17-state machine and 7 entity schemas;
market grounding: Spectr canonicalFeature `hrmq-canon-offboarding` (coverage
4/9) and the round-3 disposition analysis. The eindafrekening computation
engine, UWV WW-melding / pensioenfonds / ZVW submissions, AVG retention
timers, ExitInterview/EquipmentReturn/RetentionTimer side entities, per-item
asset tracking (the parallel `asset-management-mvp` change owns asset data —
loose coupling only) and `Employee.endDate` write automation are explicitly
out of scope.
...
New auto-discovered provider `lib/Standards/Checks/NlOffboardingChecks.php` (implements `CheckProvider`; does NOT implement `SeedsObjects`). `RuleAuditService::buildRelatedContext()` extends the existing `context['related']['Employee']` index with `endDate` and adds a `context['related']['LeaveBalance']['byEmployeeId']` index (per `employeeId`: the list of `{leaveType, year, entitledHours, bovenwettelijkHours, usedHours}` rows). Predicates, all keyed on `Offboarding`:
