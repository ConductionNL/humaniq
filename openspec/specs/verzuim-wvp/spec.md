...
- `RuleCatalogue`'s `severity` enum (rules/SCHEMA.md) is `mandatory | conditional | recommended` — there is no `advisory` severity, and `RuleEngine::violationFor()` always takes a Violation's severity from the catalogue rule, not the predicate call site, so one rule id cannot literally emit two different severities for the same object. `nl-wvp-milestone-overdue` therefore reports a single `mandatory` Violation (the only severity the corpus rule declares) whenever ANY open milestone is overdue or within the 14-day window and not Done — mirroring the exact precedent already shipped for `nl-loonaangifte-deadline-alert` / `nl-upa-deadline-alert` ("past OR within the alert window" collapses to one boolean condition). The REQ-VWP-004/006 scenario language "mandatory violation" / "advisory" describes the underlying urgency (which milestone triggered it), not two distinct machine severities — both the devries-week7 (overdue) and bakker-longterm (approaching) seed cases each report exactly one `nl-wvp-milestone-overdue` violation at `mandatory` severity.
- The seeded audit was verified by running `RuleEngine::evaluate()` directly against the three seeded `SickLeaveCase` objects (standalone PHP, no live Nextcloud/OpenRegister instance in the build environment): `sickcase-jansen-flu` reports zero violations, `sickcase-devries-week7` and `sickcase-bakker-longterm` each report exactly one `nl-wvp-milestone-overdue` violation, and no `nl-wvp-milestone-derivation` or `nl-loondoorbetaling-minimum` violations occur anywhere — matching the intended set modulo the severity-collapse noted above.
