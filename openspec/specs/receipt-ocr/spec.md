---
capability: receipt-ocr
status: done
built_by: openspec/changes/archive/2026-07-15-receipt-ocr
---
...
For an `Expense` with a non-empty `receiptFile`, `ReceiptExtractionService` SHALL call `OCA\DocuDesk\Service\FinancialExtractionService::extractFinancial($data, $requestedBy)` — `$data` carrying `fileId` (int, when `receiptFile` is purely numeric) or `documentUri` (string, otherwise) plus `docType: 'receipt'` — and, for each of `amount`/`expenseDate`/`vendor`/`vatAmount`, SHALL write the corresponding extracted value (read from the REAL response keys `totalIncl`/`issueDate`/`supplierName`/`totalVat` respectively — see the Purpose section's implementation-verified correction) onto the Expense ONLY when that field's current value is empty (per design.md D4's per-field definition: `amount === 0`, `expenseDate` absent/empty, `vendor` null/empty, `vatAmount` null) — a field already carrying a non-empty value SHALL be left unchanged regardless of what was extracted for it. The raw extracted values and `overallConfidence` SHALL be recorded on a `ReceiptExtraction` row every time, and `appliedFields` SHALL name only the fields this attempt actually wrote.
...
`ReceiptExtractionService` SHALL write extracted values onto an `Expense` via a plain field save that never sets a NEW `status` value and never invokes an `x-openregister-lifecycle` transition (`submit`/`approve`/`reject`/`reimburse`); a low-confidence or incorrect extraction SHALL therefore be able to prefill a field for human review but SHALL NEVER be able to advance the claim's lifecycle or create an approved reimbursement on its own. (Implementation note: because OpenRegister's `ObjectService::saveObject()` update path is PUT-semantic — a schema property absent from the save payload is nulled — the save payload MUST carry `status` forward at its unchanged current value rather than omitting it; omitting it would NULL the Expense's lifecycle status outright, which is a worse violation of this requirement than the one it guards against.)
