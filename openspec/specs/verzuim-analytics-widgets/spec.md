...
@e2e exclude declarative widget wiring is covered by the shared CnStatWidget/resolveFilterTokens library tests; app-level e2e suite does not exist yet (tracked by active change humaniq-test-coverage-baseline)
...
@e2e exclude declarative page filtering/sorting is covered by the shared CnIndexPage library tests; app-level e2e suite does not exist yet (tracked by active change humaniq-test-coverage-baseline)
