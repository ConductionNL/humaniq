# humaniq-dashboard-steering-indicators Specification
...
@e2e exclude manifest-shape assertion, verified by reading `pages[].widgets` directly — humaniq's e2e suite does not exist yet (tracked by active change humaniq-test-coverage-baseline)
...
@e2e exclude manifest-shape assertion, verified by reading the six widgets' `dataSource`/`endpointSource` register+schema pairs — humaniq's e2e suite does not exist yet
...
@e2e exclude declarative dataSource wiring is covered by nextcloud-vue's own useDataSource/CnChartWidget tests; humaniq's e2e suite does not exist yet
...
`GET /apps/humaniq/api/analytics/trends?metric=absence-rate`, wrapping `AbsenceRateService` per
...
@e2e exclude endpoint contract assertion, covered by a controller/service unit test asserting the raw series payload — humaniq's e2e suite does not exist yet
- **GIVEN** a period bucket with zero `EmploymentContract` availability
- **WHEN** `GET /apps/humaniq/api/analytics/trends?metric=absence-rate` resolves that bucket
- **THEN** the bucket's value in the response is JSON `null`, not `0`

#### Scenario: A period with real absence renders its actual rate
@e2e exclude endpoint contract assertion, covered by a controller/service unit test — humaniq's e2e suite does not exist yet
...
the first place in humaniq that reads `AdministrationAccess.role` past row presence.

#### Scenario: An employee-role caller is refused
@e2e exclude controller-level auth assertion, covered by an AnalyticsControllerTest — humaniq's e2e suite does not exist yet
...
`GET /apps/humaniq/api/analytics/trends?metric=payroll-cost`. The endpoint SHALL sum
...
`GET /apps/humaniq/api/analytics/trends?metric=approval-lead-time`. The endpoint SHALL compute, per
...
`GET /apps/humaniq/api/analytics/obligations`. The endpoint SHALL merge, into one row list sorted by
...
- **WHEN** `GET /apps/humaniq/api/analytics/obligations` resolves
...
For each row `GET /apps/humaniq/api/analytics/obligations` already returns, the endpoint SHALL call
...
@e2e exclude performance-shape assertion, covered by a controller/service unit test asserting the object-loading calls made — humaniq's e2e suite does not exist yet
