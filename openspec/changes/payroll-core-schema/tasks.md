...
- [ ] 1. Tables: create `lib/Standards/tables/nl-2026.json` with the complete verified 2026
...
- [ ] 2. Tables: create `lib/Standards/tables/SCHEMA.md` (shape, leaf format, euros-2-decimals,
  annual re-issue discipline, NOT-OpenRegister rationale) mirroring `rules/SCHEMA.md` per
  REQ-PCS-002
- [ ] 3. Schema: add `Employee.loonheffingskortingToegepast` (boolean, default true, description
  per design.md) + bump Employee to 0.4.0 in `lib/Settings/register.d/hr-objects.json` per
  REQ-PCS-003
- [ ] 4. Schema: add `PayrollRun.calculatedAt` (date-time, nullable) + `PayrollRun.engineVersion`
  (string, nullable) + bump PayrollRun to 0.2.0 per REQ-PCS-004
- [ ] 5. Schema: add `Payslip.arbeidskorting` (number, nullable) + `Payslip.payrollRunId` (string,
  uuid, `$ref` PayrollRun, nullable) + bump Payslip to 0.3.0 per REQ-PCS-005
- [ ] 6. Corpus: add `nl-engine-table-version` + `nl-engine-output-consistency` to
...
- [ ] 7. Corpus: bump `RuleCatalogue::VERSION` to `2026-07.6` per REQ-PCS-006
- [ ] 8. Unit guard: extend the existing Standards tests (tests/Unit/Standards/) with a tables-file
...
- [ ] 9. Quality gates: `composer check:strict` green; in the dev container re-import the register
