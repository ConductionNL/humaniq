...
Give humaniq's approved-timesheet event a consumption path a sibling Conduction app can actually use.
...
system SHALL ALSO dispatch a typed `OCA\Humaniq\Event\TimesheetApprovedEvent` through Nextcloud's
...
humaniq's `Timesheet.period` field is polymorphic-grain: `YYYY-MM` (a calendar month), `YYYY-Www` (an
