...
- [x] 1.1 In `src/manifest.json`, replace Dutch menu `label` values with English sentence-case
...
- [x] 1.2 Replace page `title` values with the matching English key (`"Urenstaat"` →
      ...
      equivalent).
- [x] 1.3 Confirm the existing English widget-content strings (`"Total hours"`, `"Claimed
...
- [x] 2.1 Create `l10n/en.json` — identity-mapped (`"translations": { "Timesheets": "Timesheets",
      ...
      library-provided via `registerTranslations()` and out of scope — only hrmq's own strings).
- [x] 2.2 Create `l10n/nl.json` with the Dutch translation for every key in `en.json` — same key
      set, zero gaps, per ADR-007.
- [x] 2.3 Confirm the Nextcloud build/packaging step serves `l10n/*.json` at
      `/custom_apps/hrmq/l10n/<locale>.json` with no additional `appinfo/info.xml` wiring needed
...
- [ ] 3.3 Confirm `loadTranslations('hrmq', ...)` in `src/main.js` no longer 404s (verify via
      browser network tab or server access log) for at least one non-default locale.
- [x] 3.4 Diff `l10n/en.json` and `l10n/nl.json` key sets — confirm they are identical (zero gaps),
      per ADR-007. Verified: both catalogues have the same 18 keys (en-only=∅, nl-only=∅), every
      `en.json` entry is identity-mapped (key === value).
