...
**Status:** in-progress
**Scope:** hrmq
**Kind:** config (declarative schema + manifest; the check-provider methods are the app's established ADR-031 exception — deliberately NO new lifecycle guard classes, see design.md D2)

**OpenSpec changes**
- `onboarding-wizard-mvp` (2026-07-12)

## Purpose

Give hrmq its first onboarding surface: one `Onboarding` case object per hire in a new `hr-onboarding` register fragment, with a simplified declarative lifecycle (`aangenomen → contract_getekend → gegevens_gevalideerd → gereed_eerste_werkdag → proeftijd_lopend → afgerond`, cancellable pre-`afgerond`), checklist boolean/date fields for the concrete legal gates, three machine-checkable NL corpus rules (WID identity check, proeftijd bewaking, loonheffingenverklaring), and onboarding pages under the new ADR-001 `Onboarding & ATS` menu group. MVP-scoped from the 2026-05 `spec/onboarding-wizard` draft; the wizard stepper UI, BSN/IBAN validation services, IT auto-provisioning and the reminder engine are explicitly out of scope.

## Requirements

### REQ-OBW-001: A new `hr-onboarding` fragment SHALL define the `Onboarding` schema
...
### REQ-OBW-002: `Onboarding` SHALL carry a declarative lifecycle `aangenomen → … → afgerond` with `annuleren` from every pre-`afgerond` state
...
### REQ-OBW-003: The rule corpus SHALL gain three machine-checkable NL onboarding rules
...
### REQ-OBW-004: `NlOnboardingChecks` SHALL enforce WID timing, proeftijd limits, and loonheffingenverklaring presence
...
### REQ-OBW-005: New onboarding pages SHALL surface the case, its checklist, and the lifecycle under the `Onboarding & ATS` menu group
...
### REQ-OBW-006: Seed data SHALL provide one mid-flow case exercising the WID violation and one clean completed case
