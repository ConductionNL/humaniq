...
- [ ] 1.1 In `lib/Controller/PageController.php::manifest()`, compute an ETag (e.g. from
...
- [ ] 1.2 Confirm Nextcloud's HTTP layer honours the ETag for conditional `If-None-Match` requests
...
- [ ] 1.3 Avoid re-reading the file from disk unnecessarily within the same request lifecycle (a
...
- [ ] 2.1 In `src/main.js`, before passing `bundledManifest` into the root `Vue` instance's props
...
- [ ] 2.2 Apply the same treatment to `pageTypesProp`/`registryProp` (`main.js:95-96`), which are
      already shallow-cloned for extensibility but not yet marked non-reactive.
- [ ] 2.3 Confirm this does not break `CnAppRoot`/`CnPageRenderer`'s internal reactive reads of
...
- [ ] 3.1 Call `GET /apps/humaniq/api/manifest` twice with `curl -H "If-None-Match: <etag-from-first-call>"`
