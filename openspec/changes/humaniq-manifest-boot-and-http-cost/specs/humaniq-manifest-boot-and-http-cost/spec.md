...
`GET /apps/humaniq/api/manifest` SHALL return caching headers appropriate for a build-time-immutable
...
- GIVEN a prior `GET /apps/humaniq/api/manifest` response with an `ETag` header
...
- WHEN `GET /apps/humaniq/api/manifest` is called after the upgrade
