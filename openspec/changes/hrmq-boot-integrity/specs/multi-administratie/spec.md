...
**Correction (2026-08-19, `hrmq-boot-integrity`):** this requirement previously read
...
rows to any authenticated caller, because no hrmq schema declares an `authorization` block and
...
@e2e exclude no e2e suite exists yet (tracked by hrmq-test-coverage-baseline); asserted at apply time against the rebuilt bundle as the positive control in tasks.md, since a negative-only assertion ("not the literal token") is also satisfied by the clause being dropped entirely — which is a different, unscoped outcome
