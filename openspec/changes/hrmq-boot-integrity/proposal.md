...
hrmq's primary index pages render "No items found" while the data exists, and two of six PHP
...
- **Diagnose and fix the unresolved-sentinel defect at its actual layer.** Traced through
  ...
  defect is neither in hrmq's provide nor in the library's current logic — it is an artefact of a
  stale, unrebuilt production bundle.** hrmq's half of the fix is therefore build hygiene, not new
  application code: get the dependency tree installed at the version the lockfile actually pins,
  rebuild `js/hrmq-main.js` from that tree, and verify the rebuilt bundle behaves as the current
...
- **A regression guard against silent-empty, at the layer that actually produces the defect.** Not
  a Playwright spec — hrmq has no e2e suite yet (tracked by the active change
  `hrmq-test-coverage-baseline`), and the defect is a pure string-substitution bug with zero DOM
  ...
  2. A bundle-freshness assertion that the shipped `js/hrmq-*.js` artefacts were built from the
...
- **`PageController::manifest()` caching and `src/main.js` prop-reactivity.** That is the active
  change `hrmq-manifest-boot-and-http-cost`'s territory. See "Relationship to other changes" below
...
- **`hrmq-manifest-boot-and-http-cost` (active): orthogonal, not superseded or absorbed.** Its
...
- **`hrmq-ia-navigation-alignment` (active): unaffected.** That change's four-leaf relocation and
  ADR-001 alignment work is unrelated to boot/build integrity.
- **`hrmq-test-coverage-baseline` (active): this change's guards are Node-level, not e2e**,
...
### New Capabilities
- `boot-integrity`: hrmq's dependency trees are verifiably installed at the versions their
...
- **No PHP or Vue application code changes.** This change's implementation surface is entirely
  ...
  `js/hrmq-*.js` once the dependency tree is correctly installed by whoever next has install
...
- **`js/hrmq-main.js` and sibling bundle chunks** — rebuilt once dependencies are correctly
