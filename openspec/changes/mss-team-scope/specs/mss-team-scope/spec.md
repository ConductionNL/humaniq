...
**Status:** in-progress
**Scope:** hrmq
**Kind:** config (declarative schema properties + manifest; the consistency predicate and audit-context extensions are the app's established ADR-031 rule-corpus exception)

**OpenSpec changes**
- `mss-team-scope` (2026-07-13)

## Purpose

Give managers a team-scoped self-service surface per ADR-001 Rule 2 (Dashboard widgets + scoped acties inside existing menus — never a portal): an optional denormalized `managerUserId` (plain NC-user-id string, the round-2 `userId` trade-off applied to the manager axis, because two-hop filtering does not exist in the manifest token grammar — re-verified at HEAD) on Timesheet/Expense/LeaveRequest, three `Team*goedkeuring` pages pre-filtered `managerUserId: @me`, the Dashboard approver widgets re-scoped to the manager's team with a global HR fallback row, and a recommended-severity machine-checkable corpus rule (`nl-mss-manager-consistency`) that cross-checks every stamp against the org structure shipped by org-chart-basic. Spectr canon: `hrmq-canon-mss-team-scope` (3/9 competitive coverage).

## ADDED Requirements

### Requirement: The approval-carrying schemas SHALL gain an optional denormalized `managerUserId` scoping property (REQ-MSS-001)

`Timesheet` (`lib/Settings/register.d/hr-timesheet.json`, version 0.3.0 → 0.4.0), `Expense` (`hr-expense.json`, 0.2.0 → 0.3.0) and `LeaveRequest` (`hr-leave.json`, 0.2.0 → 0.3.0) SHALL each declare `managerUserId`: string, nullable, NOT in `required`, NEVER a `$ref` (it names a Nextcloud *account*, mirroring the `userId`/`approvedBy` convention). Each property description SHALL state honestly that it is a denormalized copy of the `nextcloudUserId` of the manager (`OrgUnit.managerId`) of the record's employee's active `OrgAssignment` unit, populated by HR/back-office alongside `userId` because the manifest filter grammar cannot join across schemas, and audited by `nl-mss-manager-consistency`. The register `info.version` (`lib/Settings/hrmq_register.json`) bumps 0.5.0 → 0.6.0. No lifecycle change on any schema. Only these three schemas gain the field (they alone carry an approval surface).
