...
humaniq has **no MCP surface at all** today — no provider, no dialect, no
...
- **Zero write verbs. Zero curated `#[McpTool]`s. No PHP changes at all** — hence
  `kind: config`. humaniq's back-office actions are payroll runs, tax filings, contract
...
- **REFUSE the health domain**: `SickLeaveCase` (art. 9 AVG) — and, on the same ground,
  **`LeaveRequest` and `LeaveBalance`**, because humaniq models sick leave as a *value* of the
...
### New Capabilities
- `humaniq-mcp-surface` — humaniq's agent-facing tool surface: the six-schema read-only allowlist,
  the zero-write posture, and the standing refusals over remuneration, health, recruitment
  and worker-monitoring data.

### Modified Capabilities
_None._ The dialect is additive metadata on existing schemas. No humaniq requirement changes
...
- **Consumers:** Hermiq, via OpenRegister's registry. No humaniq controller, route or frontend
  change.
