...
humaniq's agent-facing MCP tool surface under ADR-063: a deliberately thin, read-only allowlist
and the standing refusals over remuneration, health, recruitment and worker-monitoring data.

## ADDED Requirements

### Requirement: humaniq declares a six-schema read-only MCP allowlist
humaniq MUST expose exactly six schemas to MCP via `configuration.x-openregister-mcp` —
...
carry `scope: "read"`, `readOnlyHint: true`, and an agent-facing `description`. No other humaniq
schema may carry an `x-openregister-mcp` block.

#### Scenario: The six allowlisted schemas expose derived read tools
- **WHEN** OpenRegister derives its tool surface from humaniq's register
- **THEN** exactly twelve derived tools exist — `humaniq.{schema}.search` and
  `humaniq.{schema}.get` for each of the six allowlisted schemas
- **AND** each reports `readOnlyHint: true` and `scope: read`

#### Scenario: No other humaniq schema is reachable
- **WHEN** an agent lists humaniq's tool surface
...
### Requirement: No humaniq schema exposes any write verb
The `create`, `update` and `delete` verbs MUST NOT be enabled on any humaniq schema, and no
humaniq service may carry `#[McpTool]`. An humaniq write is a wage payment, a tax filing to the
...
guarded by `OCA\Humaniq\Lifecycle\NoSelfApprovalGuard` — a separation-of-duties control that an
...
- **WHEN** an agent enumerates humaniq's derived tools
- **THEN** no tool name ends in `.create`, `.update` or `.delete`
- **AND** no curated two-segment humaniq tool exists at all
...
- **THEN** no MCP tool performs the transition, and the agent MUST report that approval is a
  manager action in the humaniq UI, where `NoSelfApprovalGuard` applies
...
- **AND** the agent MUST refer the user to the humaniq self-service UI
...
- **AND** the agent MUST report that applicant review is a human decision in the humaniq UI

#### Scenario: Vacancies are still readable
- **WHEN** a user asks which positions are open
- **THEN** `humaniq.Vacancy.search` answers from `title`, `department`, `status` and
...
- **WHEN** humaniq's register and its `register.d` fragments are imported into OpenRegister
...
- **AND** the total humaniq schema count is still 23
