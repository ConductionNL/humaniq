...
- [ ] 3.1 `python3 -m json.tool` on each of the five touched fragments; all valid, humaniq schema count still 23, every pre-existing `x-openregister-lifecycle` block byte-for-byte unchanged.
