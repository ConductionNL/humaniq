...
approved hrmq Timesheet that is not an admin-configured outbound HTTP webhook. hrmq's existing
...
synchronous result slot, because the producer does not need an answer back. hrmq's approval event is
...
- **New `OCA\Hrmq\Event\TimesheetApprovedEvent`** (`lib/Event/TimesheetApprovedEvent.php`) — a typed
...
- **shillinq's consuming listener.** Building the `IEventListener` that consumes this event and
  ...
  job — out of scope here per this change's own brief. This change only makes hrmq's side consumable.
- **Resolving the period-grain question.** hrmq's `Timesheet.period` is polymorphic-grain
...
### New Capabilities
- `hrmq-timesheet-approved-typed-event`: a typed, `IEventDispatcher`-dispatched cross-app event
  (ADR-041) fired alongside hrmq's existing approved-timesheet webhook, giving a sibling Conduction
...
- **No schema change, no DB, no direct SQL** (ADR-022) — this change adds no new persisted state; it
  only widens hrmq's existing approval-event emission surface.
- **shillinq's `consume-hrmq-events` change is unblocked** by this change landing — it can now
  `class_exists()`-guard `\OCA\Hrmq\Event\TimesheetApprovedEvent` and register an `IEventListener`
