...
Humaniq ships zero automated tests of any kind. Verified at HEAD:
...
- `composer.json`'s `autoload-dev` declares `"OCA\\Humaniq\\Tests\\": "tests/"` (`composer.json:19-21`)
...
- Scaffold the missing `tests/` directory the codebase already references but never created:
  `phpunit.xml` (Nextcloud app conventions, `OCA\Humaniq\Tests\` namespace matching
...
- Add a minimal Playwright e2e spec (matching the fleet's Playwright-UI-only convention) that
  drives the real `Timesheets` index page end-to-end: navigate to `/apps/humaniq/timesheets`, assert
...
### New Capabilities
- `humaniq-test-coverage-baseline`: a minimal but real automated test baseline exists — pure
...
- **New Playwright e2e project** (or addition to an existing fleet-shared one, if humaniq already has
