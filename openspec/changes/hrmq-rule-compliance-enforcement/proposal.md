...
hrmq's own architecture doc says the compliance engine has two halves: "the predicates are
side-effect free and unit-tested; the lifecycle wiring + object loading live in
`OCA\Hrmq\Lifecycle\RuleComplianceGuard`" (`lib/Standards/RuleEngine.php:19-20`). That class does
...
is wired only to the read-only `occ hrmq:rules:audit` command
...
Put together: hrmq ships an app whose store description and summary lead with "built-in
...
manually runs `occ hrmq:rules:audit`. Compounding this: none of those five schemas
...
- Make the read-only audit **actionable** without requiring new OpenRegister guard-hook
  infrastructure: `occ hrmq:rules:audit` SHALL exit with a non-zero status code when any
...
- **Design decision required** (see `design.md`): whether hrmq should (a) add a minimal
...
### New Capabilities
- `hrmq-rule-compliance-enforcement`: the compliance audit surfaces mandatory violations as an
