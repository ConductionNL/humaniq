...
- [x] 1.3 Switch `<website>`/`<documentation>`/`<discussion>`/`<bugs>`/`<repository>`/
      `<screenshot>` from the GitHub mirror to `codeberg.org/Conduction/hrmq` (README states the
      GitHub repo is now read-only).
- [x] 1.4 Leave `<version>0.1.0</version>` unchanged — it is the cross-surface source of truth.

## 2. Product page (`conduction-website/src/pages/apps/hrmq.mdx` + NL i18n)

- [x] 2.1 Author `conduction-website/src/pages/apps/hrmq.mdx` (EN) from the `apps/shillinq.mdx`
...
- [x] 2.2 Author `conduction-website/i18n/nl/docusaurus-plugin-content-pages/apps/hrmq.mdx` (NL)
      as a real Dutch translation (not a copy of the EN copy).
- [x] 2.3 Add an `hrmq` entry to `conduction-website/src/data/apps-catalog.js` `PRESENTATION`
      (`categories: ['Processes']`) so the new page surfaces on `/apps` instead of being
      unreachable without a direct URL.
- [x] 2.4 Add an `hrmq: 'HR'` monogram to `conduction-website/src/components/AppGlyph/AppGlyph.jsx`.

## 3. Docs site — flagged, not scaffolded

- [ ] 3.1 Decide whether to invest in a `docs/` Docusaurus site for HRMQ via `journeydoc-init`
      (app-owner decision — out of scope for this alignment pass; see proposal.md "Still
      misaligned").

## 4. Follow-ups outside this repo

- [ ] 4.1 File/track a change against `docusaurus-preset` to add an `hrmq` entry to its shipped
