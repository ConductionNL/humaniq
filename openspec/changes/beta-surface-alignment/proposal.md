...
HRMQ has no public product page (`conduction-website/src/pages/apps/hrmq.mdx` did not exist, EN or
...
EmploymentContract / PayrollRun / Payslip / WageTaxFiling) as if HRMQ shipped UI for them. Before a
...
3. **HR/payroll compliance rule engine** — `lib/Standards/RuleEngine.php` +
   ...
   Zvw) via `occ hrmq:rules:audit` (`lib/Command/RulesAuditCommand.php`).
4. **`occ hrmq:rules:seed-testdata`** — idempotent local test-data seeding
...
5. **OpenRegister-backed data layer** — Employee, EmploymentContract, PayrollRun, Payslip,
   ...
   (`lib/Settings/hrmq_register.json` + `register.d/*.json`, imported by
   `OCA\Hrmq\Repair\InitializeRegister`); HRMQ owns no database tables of its own.
...
1. **`hrmq/appinfo/info.xml`**
   ...
   - `<website>`/`<documentation>`/`<discussion>`/`<bugs>`/`<repository>`/`<screenshot>`: switched
     from `github.com/ConductionNL/hrmq` to `codeberg.org/Conduction/hrmq` — the GitHub repo's own
     `README.md` states it "has moved to Codeberg" and is now a read-only mirror; info.xml was
     still pointing users/reviewers at the mirror.
   - `<dependencies>`: added an explanatory XML comment declaring the OpenRegister dependency (NC's
     ...
     `"dependencies": ["openregister"]`, and HRMQ has no own controllers beyond the SPA shell — all
...
2. **`conduction-website/src/pages/apps/hrmq.mdx`** (new) and
   **`conduction-website/i18n/nl/docusaurus-plugin-content-pages/apps/hrmq.mdx`** (new) — authored
...
3. **`conduction-website/src/data/apps-catalog.js`** and
   **`conduction-website/src/components/AppGlyph/AppGlyph.jsx`** — added an `hrmq` entry
...
4. **`hrmq/src/manifest.json` / `manifest.d/`** — no changes. The nav labels (`Uren`,
...
| `occ hrmq:rules:audit` / `occ hrmq:rules:seed-testdata` | Yes — `lib/Command/*`, registered in `info.xml` `<commands>` | Kept |
...
| Any Peppol/SEPA/BBV/DigiD-style compliance mark | Not found anywhere in `lib/`/`src/`/README | None made; not applicable to this app (that vocabulary belongs to Shillinq, not HRMQ) |
| Shillinq/DocuDesk/LaunchPad live integration | Not found — one schema comment explicitly disclaims it as unbuilt | Not claimed |

### Icon status

`hrmq/img/app.svg` (white fill, `viewBox="0 0 24 24"`, no background — brand convention),
...
- **`@conduction/docusaurus-preset`'s shipped `apps-registry.js`** (consumed by `<AppCrossLinks/>`
  and the academy product filter) has no `hrmq` entry. That file lives in the preset package
  ...
  update both.").
- **App Store listing**: per the 2026-07-07 fleet App Store audit, HRMQ was among the 14 apps not
