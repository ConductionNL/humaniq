...
Compensation review cycles on the personnel dossier (ADR-001 Rule 6): a `SalaryBand` reference model,
a `CompReviewCycle` round container, a per-employee `CompAdjustment` proposal carrying the declarative
draft→proposed→approved→effective lifecycle (separation-of-duties + effective-date guards), the
imperative effective-dated write onto the employee's compensation via occ command and one guarded
endpoint, a within-band CheckProvider, and the dossier detail surface + salary-band reference
sub-pages. External market-data benchmarking is out of scope.
...
onto `EmploymentContract.hourlyWage` for hourly employees is a named fast-follow.
