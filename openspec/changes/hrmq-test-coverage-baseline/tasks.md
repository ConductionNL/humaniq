...
- [x] 1.1 In `lib/Standards/RuleEngine.php`'s class docblock, remove "The predicates are
...
- [x] 2.1 Create `tests/` with `phpunit.xml` (Nextcloud app conventions: bootstrap, `Unit` suite,
      matching the `OCA\Hrmq\Tests\` namespace already declared in `composer.json`'s
      `autoload-dev`).
- [x] 2.2 Confirm `composer test:unit` / `composer test:all` (currently silently no-op with no
      `tests/` to discover) actually run the new suite and exit non-zero on a failing assertion.
      Verified via PHPUnit 10.5 (`phpunit -c phpunit.xml`): the `Unit` suite is discovered and
      runs 25 tests / 1066 assertions, PASS. `composer test:unit` invokes this same
      `./vendor/bin/phpunit`; the app's vendor tree is not installed in this worktree, so the
      binary was supplied from a sibling app — the phpunit.xml + bootstrap + suite are proven.

## 3. Unit-test the pure compliance-predicate layer

- [x] 3.1 `tests/Unit/Standards/RuleCatalogueTest.php` — `count() === count(all())`; every entry in
      ...
      string.
- [x] 3.2 `tests/Unit/Standards/RuleEngineTest.php` — jurisdiction scoping per the class docblock's
...
- [x] 3.3 `tests/Unit/Standards/Checks/NlWageTaxFilingChecksTest.php` — `onOrBefore()` boundary
...
- [x] 4.1 Add a Playwright spec (following the fleet's UI-only e2e convention — no API-direct
      assertions) that navigates to `/apps/hrmq/timesheets`, waits for the manifest-driven
      ...
      isolation.
- [x] 4.2 Wire the new Playwright project into whatever fleet-shared e2e runner convention the
...
- [x] 5.1 `package.json`'s `check:manifest` currently runs `node tests/validate-manifest.js`, which
...
- [x] 6.1 Run `composer test:unit` — confirm it discovers and runs the new tests (not the previous
      silent "Tests require Nextcloud environment, skipping..." fallback with zero tests found).
      Verified equivalently: `phpunit -c phpunit.xml` discovers the `Unit` suite and runs 25 tests
      (not zero) — see 2.2. (`composer test:unit` needs the app's own `vendor/` installed.)
- [ ] 6.2 Run the new Playwright spec against a local dev instance — confirm it fails if the
      ...
      no-op).
- [x] 6.3 Run `npm run check:manifest` — confirm it no longer errors on a missing file.
