...
Two independent, verifiable boot/request-cost gaps in how HRMQ handles its own static manifest:
...
requires it"), this route is scaffolded ahead of hrmq's own frontend consuming it — verified:
...
frontend (`src/registry.js` is deliberately empty) — the manifest *is* hrmq's frontend, so its
...
### New Capabilities
- `hrmq-manifest-boot-and-http-cost`: the bundled-manifest HTTP endpoint is cacheable, and the
