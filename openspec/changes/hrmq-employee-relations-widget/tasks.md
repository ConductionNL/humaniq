...
- [x] 1.1 Add an `Employee` seed block to `lib/Settings/register.d/hr-seed.json` (or a new
      ...
      `hr-objects.json`). Done: new fragment `lib/Settings/register.d/hr-employee-seed.json` with
      three objects carrying stable slugs AND fixed `@self.id` UUIDs (so the related-object field
...
- [ ] 1.2 Confirm the three new `Employee` objects import cleanly via
      ...
      validation errors. [needs live instance]

## 2. Declare the relation on Timesheet/Expense (config)

- [x] 2.1 Update `employeeId` in `lib/Settings/register.d/hr-timesheet.json` to declare an
      OpenRegister object reference to the `Employee` schema (follow the fleet's declarative
      `$ref` + `objectConfiguration.handling: related-object` reference pattern rather than a bare
      `type: string`, per ADR-022 — matching the softwarecatalog single-object relation pattern).
- [x] 2.2 Update `employeeId` in `lib/Settings/register.d/hr-expense.json` the same way.
- [x] 2.3 Update the six existing seed rows in `hr-seed.json` (three `Timesheet`, three `Expense`)
      to reference the real seeded `Employee` UUIDs from task 1.1 instead of the placeholder
...
- [x] 4.1 Update the `_note` fields in `src/manifest.json` (`ts-related`/`ex-related`) if the
      ...
      Done: both `_note`s now state `employeeId` is a resolvable OR relation and that
      `projectId`/`costCenter` remain free-text business keys.
