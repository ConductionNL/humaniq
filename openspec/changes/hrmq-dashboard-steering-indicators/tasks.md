...
- [x] 4.2 Add the Billable-ratio `chart` widget: `dataSource` raw-GraphQL form, two aliased categorical `Timesheet.period` group-bys (billable-filtered sum + unfiltered sum), `administrationId: "@workspace.activeAdministrationId?"` (REQ-DSI-002)
- [x] 4.3 Add the Headcount & turnover `chart` widget: `dataSource` raw-GraphQL form, two aliased `Employee` time-bucket group-bys (`startDate`/`endDate`, `interval: MONTH`, `metric: COUNT`), same tenant filter (REQ-DSI-003)
