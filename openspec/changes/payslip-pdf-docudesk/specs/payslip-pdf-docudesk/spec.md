# Spec: payslip-pdf-docudesk

**Status:** in-progress
**Scope:** hrmq (loonstrook + jaaropgaaf documents through the existing docudesk consumption leaf)
**Kind:** code (service aggregation/generation paths + command options + controller guard + check provider dominate; schema fields, one rule row, and manifest pages ride along — see design.md "Mixed-spec rationale")

**OpenSpec changes**
- `payslip-pdf-docudesk` (2026-07-13)
