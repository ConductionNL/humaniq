# Spec: payroll-glpost-shillinq

**Status:** in-progress
**Scope:** hrmq (cross-app leaf consuming the shillinq `JournalEntry` register via OpenRegister)
**Kind:** code (PHP service + occ command dominate; schema fragment, corpus rule, and manifest pages ride along — see design.md "Mixed-spec rationale")

**OpenSpec changes**
- `payroll-glpost-shillinq` (2026-07-12)

## Purpose

Post one balanced loonjournaalpost per approved `PayrollRun` into shillinq's `JournalEntry` register (same instance, via OpenRegister's ObjectService — never HTTP), record the handoff in a new `PayrollGLPost` object, turn the existing GL-reconciliation corpus rules green, and degrade gracefully to `skipped-no-shillinq` when the bookkeeping app is absent.
