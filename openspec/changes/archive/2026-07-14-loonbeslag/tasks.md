...
- [x] 1. Schema: NEW `lib/Settings/register.d/hr-loonbeslag.json` — `Loonbeslag` (creditor,
...
- [x] 2. Schema: `Payslip.loonbeslag` (nullable number, euro) + `Payslip.loonbeslagId` (nullable
  `$ref` Loonbeslag) in `lib/Settings/register.d/hr-objects.json` per REQ-BESLAG-004
- [x] 3. Service: `PayrollRunService::activeLoonbeslagenByEmployeeId(period)` — the
...
- [x] 4. Service: the floor-clamp fold — `deductionCents = min(orderedAmountCents, max(0,
  ...
  REQ-BESLAG-002/-004
- [x] 5. Controller: NEW `lib/Controller/LoonbeslagController.php` — `activate()`/`settle()`/
...
- [x] 6. Routes: `appinfo/routes.php` — 3 new routes (`/api/loonbeslag/activate`,
  `/api/loonbeslag/settle`, `/api/loonbeslag/withdraw`) registered BEFORE the SPA catch-all per
  REQ-BESLAG-006
- [x] 7. Checks: NEW `lib/Standards/Checks/NlLoonbeslagChecks.php` —
...
- [x] 8. Checks: `nl-loonbeslag-single-active` (Loonbeslag, flags >1 `actief` with overlapping
  effective ranges per employee) + `RuleAuditService::audit()` `payroll.loonbeslagenById` context
  enrichment per REQ-BESLAG-005/-007
- [x] 9. Corpus: register `nl-loonbeslag-beslagvrije-voet-floor` + `nl-loonbeslag-single-active` in
...
- [x] 10. Manifest: `Loonbeslagen` index page + `LoonbeslagDetail` (three guarded `api-call` page
...
- [x] 11. Seed: one `Loonbeslag` for the existing seeded employee exercising the CLAMPED branch
  (`orderedAmount` exceeding headroom above `beslagvrijeVoet`) per design.md Seed Data
- [x] 12. Tests: `PayrollRunServiceTest` — floor-clamp cases (clamped, unclamped, zero-headroom),
...
- [x] 13. Tests: `NlLoonbeslagChecksTest` — both predicates including vacuous scopes (null
  `loonbeslagId`, single active, zero active) per REQ-BESLAG-007
- [x] 14. Tests: `LoonbeslagControllerTest` — 403 non-admin/HR caller (before any resolve), 404
  unknown/unauthorized id, 400 wrong-status precondition, happy path for each of activate/settle/
  withdraw per REQ-BESLAG-006
- [x] 15. README: beslagvrijeVoet-is-a-stored-input limitation + single-active-beslag MVP scope +
  the two named follow-ups (computed voet, multi-beslag priority ordering) per REQ-BESLAG-003/-005
- [x] 16. Quality gates: `composer lint` green, full PHPUnit suite green, `npm run check:manifest`
