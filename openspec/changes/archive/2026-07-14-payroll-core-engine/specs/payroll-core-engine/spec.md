# Delta — payroll-core-engine

Chain spec 2 of 2 (ADR-032, `depends_on: [payroll-core-schema]`): the pure table-driven NL payroll
calculator, the draft-run generation service, occ run/verify commands, the guarded calculate
endpoint + run-pages upgrade, enforcement of the spec-1 engine contract, golden tests and the
