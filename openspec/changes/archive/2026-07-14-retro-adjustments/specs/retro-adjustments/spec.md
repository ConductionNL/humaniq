# Delta — retro-adjustments

Retroactive payroll adjustments (terugwerkende kracht herrekening / TWK) plus a year-transition
procedure, built on the merged `payroll-core-engine`: a `PayrollAdjustment` delta model, a service
that recomputes a sealed period against its own tax year and diffs a cents-exact delta idempotently
by `correctionRef`, the surfacing of that delta as a current-run payslip component (never a history
mutation), occ commands, one guarded endpoint, and the corpus self-check.
