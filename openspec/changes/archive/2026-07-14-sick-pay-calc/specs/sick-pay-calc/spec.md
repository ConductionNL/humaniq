...
The sick-pay (loondoorbetaling bij ziekte) computation over the merged payroll engine: a pure
integer-cents `SickPayCalculator` (statutory 70%, year-1 minimum-wage floor, wachtdag deduction, CAO
uplift percentage, samengesteld/aangepast loon), its `PayrollRunService` integration so a sick
employee's payslip reflects doorbetaald loon, the `nl-loondoorbetaling-floor` corpus rule + check,
and a hand-computed golden fixture with idempotency.
