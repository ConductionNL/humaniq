# Delta — proforma-payslip

A persist-nothing front door to the existing `payroll-core-engine` calculator: a stateless proforma
service, one guarded endpoint, one occ command, and a manifest "Simuleer loonstrook" surface. The
`PayrollCalculator`, `CalculationInput`, `CalculationResult` and `TaxTables` are reused AS-IS — this
change adds no tax logic, no tables and no schema.
