# HRMQ Timesheet Approval

**Spec refs**: `hrmq-timesheet-approval`, ADR-022 (apps consume OR abstractions), ADR-031/ADR-001 (declarative-first), ADR-036 (declarative manifest pages)
**Standards**: Schema.org (`schema:Action`); HR/labour separation-of-duties

## ADDED Requirements

### Requirement: Timesheet object with declarative approval lifecycle

The system SHALL model a submitted timesheet as a `Timesheet` object in the OpenRegister `hrmq`
