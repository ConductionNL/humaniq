# Delta — payroll-core-schema

Chain head (spec 1 of 2, ADR-032): the versioned NL 2026 tax-year parameter corpus, the minimal
Employee/PayrollRun/Payslip schema deltas the payroll engine reads and writes, and the two corpus
rules pinning the engine's traceability + output-consistency contract. `payroll-core-engine`
(kind: code) depends on this capability.
...
to `2026-07.6`. The declared net equation of `nl-engine-output-consistency` is
