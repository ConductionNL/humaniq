...
**Spec refs**: `hrmq-expenses`, `hrmq-timesheet-approval` (shared `NoSelfApprovalGuard` + SPA shell), ADR-022 (apps consume OR abstractions), ADR-031/ADR-001 (declarative-first), ADR-036 (declarative manifest pages)
**Standards**: Schema.org (`schema:Invoice`); HR/labour separation-of-duties
...
OCA\Hrmq\Lifecycle\NoSelfApprovalGuard` — the same OpenRegister `LifecycleGuardInterface` the
