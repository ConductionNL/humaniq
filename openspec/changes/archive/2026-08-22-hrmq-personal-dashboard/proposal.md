...
"Mijn HR" is humaniq's personal surface — and today it is a dead group header. The base manifest
...
humaniq's effective `main` menu today carries **7** top-level entries (Dashboard, MijnHrGroup,
...
moves humaniq to 5 counted entries and makes the exemption real rather than claimed. This is also
...
`humaniq-dashboard-steering-indicators` rebuilt it as a management steering page and **removed** the
...
Labels stay **Dutch literals** — `humaniq-i18n-locale-completeness` lands after this change and owns
the key conversion; this change adds no English keys and converts nothing.

## Capabilities

- **Added**: `humaniq-personal-dashboard` — the caller-scoped personal landing dashboard and the
...
- **Ordering**: lands **after** `humaniq-hours-process-redesign` — the hours widgets bind to its
  ...
  `humaniq-i18n-locale-completeness`.

## Non-Goals

- **No change to the management `/dashboard`** — `humaniq-dashboard-steering-indicators` owns it;
...
- **No label/i18n conversion** (owned by `humaniq-i18n-locale-completeness`).
