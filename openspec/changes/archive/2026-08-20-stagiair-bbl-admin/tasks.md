...
- [x] 1. Schema: NEW fragment `lib/Settings/register.d/hr-stagiair.json` — `Stagiair`
  ...
  REQ-STAG-001
- [x] 2. Schema: `EmploymentContract.type` enum gains `bbl` (`hr-objects.json`); NEW nullable
...
- [x] 3. Register: `lib/Settings/hrmq_register.json` `info.version` bump (new fragment picked up by
  the existing Repair import, no code change)
- [x] 4. Corpus: add `nl-bpv-overeenkomst-vereist` to `lib/Standards/rules/labour.json` (framework
...
- [x] 5. Checks: NEW `lib/Standards/Checks/NlStagiairChecks.php` — predicate registered under both
  `Stagiair` and `EmploymentContract` (guarded `type === 'bbl'`) per REQ-STAG-004/-005; auto-
  discovered, no registration step
- [x] 6. Manifest: `Stagiairs` index (columns: naam via begeleider link is not available — list
  ...
  passes (Ajv, 0 errors)
- [x] 7. Seed: `stagiair-devries` (compliant, BPV signed) + `stagiair-bakker` (intended violation,
...
- [x] 8. Tests: `NlStagiairChecksTest` — Stagiair compliant/violation/future-startDate-vacuous cases;
...
- [x] 9. Tests: end-to-end `RuleAuditServiceTest` (or equivalent) confirming the seeded audit
  reports exactly one new violation (`stagiair-bakker`) and zero regressions on existing rules
- [x] 10. README: the BPV-signing boundary (HR-entered boolean, not e-signature — cites offer-esign
  design.md point 4) and the SBB/RVO/evaluation-scheduling out-of-scope note per proposal Non-goals
- [x] 11. Quality gates: `composer check:strict` (phpcs/phpmd/psalm/phpstan/phpunit) all green;
