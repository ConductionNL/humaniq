# Delta — stagiair-bbl-admin

Modernises the 2026-05 `spec/stagiair-bbl-admin` draft against current HEAD: a standalone
`Stagiair` schema kept structurally outside `Employee`/payroll, a `bbl` `EmploymentContract.type`
value for BBL-leerlingen (who already have a real arbeidsovereenkomst and need no second entity),
an honestly-scoped stagevergoeding fiscal note, an HR-entered BPV-signing boolean (not an
e-signature flow — the shipped `offer-esign` leaf already proved that path does not reach external
signers), and one new machine-checkable rule.
