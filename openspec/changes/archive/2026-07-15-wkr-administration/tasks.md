...
- [x] 1. Table data: add the `wkr` parameter group to `lib/Standards/tables/nl-2026.json`
  ...
  per REQ-WKR-002
- [x] 2. Corpus: add rule `nl-wkr-eindheffing-exposure` (machineCheckable, conditional,
...
- [x] 3. Version: bump `RuleCatalogue::VERSION` (table + rule change) per REQ-WKR-002
- [x] 4. Schema: add `WkrDeclaration` to `lib/Settings/register.d/hr-objects.json` (administrationId,
  year, date, description, amount, `wkrCategory` enum, nullable `employeeId` `$ref`, sourceReference)
  per REQ-WKR-001
- [x] 5. Schema: add `WkrAssessment` to the same fragment (fiscaleLoonsom, vrijeRuimte,
...
- [x] 6. Context: add `RuleAuditService::buildWkrContext()` — `[administrationId][year] => {loonsom,
...
- [x] 7. Context: inject `$context['wkr'] = buildWkrContext()` in both `audit()` and
  `auditPayrollRunScope()` per REQ-WKR-004
- [x] 8. Check: NEW `lib/Standards/Checks/NlWkrChecks.php` (`CheckProvider`) registering
...
- [x] 9. Service: NEW `lib/Service/WkrService.php` `assess(administrationId, year)` — tranche
...
- [x] 10. Service: `--all` path — iterate distinct (administrationId, year) pairs across payslips per
  REQ-WKR-005
- [x] 11. Command: NEW `lib/Command/WkrAssessCommand.php`
  (`hrmq:wkr:assess --administration ADM --year YYYY [--all]`, outcome print) + register in
  `appinfo/info.xml` per REQ-WKR-005
- [x] 12. Manifest: `WkrDeclarations` index + `WkrDeclarationDetail` (allowCreate:true) under
  `PayrollGroup` per REQ-WKR-006
- [x] 13. Manifest: `WkrAssessments` index + `WkrAssessmentDetail` (stat KPIs + "Beoordelen" action)
  under `PayrollGroup`, no new top-level menu (ADR-001); `npm run check:manifest` passes per
  REQ-WKR-006
- [x] 14. Seed: one `WkrDeclaration` + one `WkrAssessment` (within-vrije-ruimte, zero violations)
  consistent with the seeded Payslip per REQ-WKR-001/-WKR-003
- [x] 15. Tests: `tests/Unit/Service/WkrServiceTest.php` (mocked ObjectService: tranche math, idempotent
  upsert, used>available → status/eindheffingDue, `--all`) per REQ-WKR-003
- [x] 16. Tests: `tests/Unit/Standards/NlWkrChecksTest.php` (within-budget satisfied, over-budget +
...
- [x] 17. Quality gates: `composer lint` + full PHPUnit green, `npm run check:manifest` PASS,
