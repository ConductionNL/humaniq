...
- [ ] 1. New fragment `lib/Settings/register.d/hr-okr.json` (`x-hrmq-fragment: hr-okr`) scaffold
  with the `components.schemas` container per REQ-OKR-001/-002
- [ ] 2. `Objective` schema: slug/icon/version/title/description, required `title`+`ownerType`,
  ...
  per REQ-OKR-001/-005
- [ ] 3. `Objective` declarative `x-openregister-lifecycle` on `status`
  (`concept → actief → afgesloten`, terminal `afgesloten`, transitions `activeren`/`afsluiten`,
  no guard) per REQ-OKR-001
- [ ] 4. `Objective` `uitkomst` nullable enum (`behaald|deels-behaald|niet-behaald`) kept separate
  from `status`, plus `progress` number (writer-maintained) and `userId` denorm string per
  REQ-OKR-001/-003
- [ ] 5. `KeyResult` schema: slug/icon/version/title, required `objectiveId` (`$ref Objective`) +
...
- [ ] 6. Document the D3 rationale in the `progress` property descriptions on BOTH schemas
...
- [ ] 7. Manifest: `EmployeeDetail` `emp-objectives` object-list row ("Doelen & OKR's",
...
- [ ] 8. Manifest: `ObjectiveDetail` page (route `/objectives/:id`, not a menu child): `data`
...
- [ ] 9. Manifest: `KeyResultDetail` page (route `/key-results/:id`, not a menu child): `data`
  widget + `related` resolving `objectiveId` per REQ-OKR-004
- [ ] 10. Manifest: `ReviewCycleDetail` `rc-objectives` object-list ("OKR's in deze cyclus", filter
  `cycleId: @objectId`, `rowRoute: ObjectiveDetail`) — the review-period tie per REQ-OKR-004
- [ ] 11. Manifest: `MijnDoelen` index page (route `/mijn/doelen`, filter `userId: @me`, cols
  `title`/`status`/`progress`/`cycleId`) — mijn-hr self-service per REQ-OKR-005
- [ ] 12. Manifest: `deepLinks` for `Objective` and `KeyResult`; `src/icons.js` registers
  `BullseyeArrow` + `TargetVariant`; NO new menu group / top-level entry (ADR-001 Rule 6) per
  REQ-OKR-004/-005
- [ ] 13. Seed data in `lib/Settings/register.d/hr-seed.json`: one `actief` `Objective` for the
...
- [ ] 14. Validate: `npm run check:manifest` passes; the register fragment loads (schemas import,
  seed objects resolve their `$ref`s) per REQ-OKR-006
- [ ] 15. Quality: fragment JSON is valid and re-validated after any merge; schema property
