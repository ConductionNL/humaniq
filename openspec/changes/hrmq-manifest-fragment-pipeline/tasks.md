...
- [ ] 1.1 Dump `pages[].{id,route}` from the current (pre-change) `src/manifest.json` (113 entries)
...
- [ ] 1.2 Dump the current `menu[]` tree (11 top-level groups, 64 navigable entries, full
      parent/child structure) to a second fixture file, same reason.
- [ ] 1.3 Record `node tests/validate-manifest.js` output verbatim (page count, error count) and
...
- [ ] 2.1 Create `src/manifest.d/` with `_placeholder.json` (`{"pages":[],"menu":[]}`) and a
...
- [ ] 2.2 Create `src/menu-layout.json`: `_meta` (spdx-license/copyright + description matching the
...
- [ ] 3.1 For each of the 28 schema-mapped domains, create `src/manifest.d/hr-<domain>.json`
...
- [ ] 3.2 Create `src/manifest.d/hr-administratie.json` for the one content-mapped page
      (`Administraties`).
- [ ] 3.3 Reduce `src/manifest.json` to the shell: `$schema`, `version`, `dependencies`,
...
- [ ] 3.4 Grep every file under `src/manifest.d/` for `"deepLinks"`, `"runtime"`, `"dependencies"` —
...
- [ ] 4.1 Author the index-page template (`indexScaffold`) in a shared location (either
...
- [ ] 4.2 Convert all 60 index pages to `pageInstances[]` entries referencing `indexScaffold`, moved
...
- [ ] 4.3 Author the four detail templates (`simpleDetailScaffold`,
      ...
      `{{param}}`).
- [ ] 4.4 Convert the 19 covered detail pages to `pageInstances[]` in their owning domain fragments.
- [ ] 4.5 Confirm the other 30 detail pages (including `EmployeeDetail`) and the 4 base pages remain
...
- [ ] 5.1 For each of the 94 non-templated pages, confirm its `_note` moved unchanged into the
      domain fragment now holding it (part of task 3.1's page move — this is a verification pass,
      not new authoring).
- [ ] 5.2 For each of the four detail templates, write one `_note` on the `pageTemplate` capturing
...
- [ ] 5.3 For each of the 19 templated pages' original `_note`, extract whatever is genuinely
...
- [ ] 5.4 Diff-review: every `_note` string present in the pre-change `src/manifest.json` is
...
- [ ] 6.1 Add the webpack `require.context('./manifest.d/', false, /\.json$/)` fragment collection
      (mirror pipelinq's `src/main.js`, including its `/* global require */` comment explaining why
      this is not a `no-undef` lint bug).
- [ ] 6.2 Replace the direct `bundledManifest` consumption with
...
- [ ] 6.3 Confirm `routesFromManifest()` and the router catch-all (`redirect: '/timesheets'`) are
...
- [ ] 7.1 Write a Node verification script (`tests/verify-manifest-parity.js` or similar) that: loads
...
- [ ] 7.2 Extend the same script (or a second one) to assert every expanded `pageInstance`'s full
...
- [ ] 7.3 Add a route-resolution check for the six static/dynamic collision pairs
...
- [ ] 7.4 Run all three checks; fix any discrepancy by correcting the fragment/template content (not
      by loosening the check).

## 8. Fix the manifest validator's coverage (design.md Decision 6)

- [ ] 8.1 Update `tests/validate-manifest.js` to build the effective manifest (require fragments,
...
- [ ] 8.2 Run `node tests/validate-manifest.js`; confirm it reports validating 113 pages (not ~4) and
      exits 0 with 0 Ajv errors.
- [ ] 8.3 Re-run `node tests/validate-widget-keys.js`; confirm it fails identically to the task 1.3
...
- [ ] 9.1 Run `openspec validate --strict` (or equivalent) against this change's artifacts.
- [ ] 9.2 Flag to the orchestrator: `openspec/changes/hrmq-ia-navigation-alignment/` is superseded by
      ...
      superseded.
- [ ] 9.3 Flag to the orchestrator (informational, no action required by this change): the identical
