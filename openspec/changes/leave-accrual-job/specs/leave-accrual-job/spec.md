# Delta — leave-accrual-job

The automatic producer for the leave-verzuim `LeaveBalance` ledger: a monthly `TimedJob` that
provisions each active employee's current-year holiday balance with the full statutory (BW 7:634)
entitlement and accrues the bovenwettelijk opbouw slice, idempotently per `(employee, year,
month)`, written through OpenRegister's ObjectService — keeping every existing mandatory leave rule
green.
...
a background-jobs block in `appinfo/info.xml` (the app has none today). Each run SHALL enumerate
