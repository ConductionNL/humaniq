...
- [ ] 1. Schema delta: add the additive **nullable** `lastAccruedPeriod` (`YYYY-MM`, type string,
...
- [ ] 2. Config getters in `lib/Service/SettingsService.php`: `isLeaveAccrualEnabled()`
...
- [ ] 3. NEW `lib/BackgroundJob/LeaveAccrualJob.php` extending `OCP\BackgroundJob\TimedJob`:
...
- [ ] 4. `run()`: early-return no-op when `isLeaveAccrualEnabled()` is false per REQ-ACCR-005
- [ ] 5. Active-employee enumeration: `loadAll('Employee')` filtered by
  `coversPeriod(startDate, endDate, currentPeriod)` (the PayrollRunService idiom) per REQ-ACCR-001
- [ ] 6. Covering-contract resolution + `hoursPerWeek` read (`coveringContract`, EmploymentContract
  `hoursPerWeek`); skip + count employees with no contract / no `hoursPerWeek` / no identity per
  REQ-ACCR-005
- [ ] 7. Balance probe: find the `(employeeId, year, leaveType: holiday)` LeaveBalance via
  ObjectService filter (probe-before-write) per REQ-ACCR-004
- [ ] 8. First-accrual provisioning (absent balance): create with `entitledHours = 4 × hoursPerWeek`,
...
- [ ] 9. Monthly opbouw (present balance, different period): `bovenwettelijkHours += round1(
  ...
  REQ-ACCR-003
- [ ] 10. Idempotency guard: present balance whose `lastAccruedPeriod === currentPeriod` → no-op
  (no write) per REQ-ACCR-004
- [ ] 11. Persist via ObjectService `saveObject(object, register: hrmq, schema: 'LeaveBalance')`;
  never-throw degradation with logged warning per REQ-ACCR-001
- [ ] 12. Log summary per run: counts of provisioned / accrued / skipped(+reason) employees per
  REQ-ACCR-005
- [ ] 13. Register the job: NEW `<background-jobs><job>OCA\Hrmq\BackgroundJob\LeaveAccrualJob</job>
  </background-jobs>` block in `appinfo/info.xml` per REQ-ACCR-001
- [ ] 14. Tests: `tests/Unit/BackgroundJob/LeaveAccrualJobTest.php` (mocked ObjectService) —
...
- [ ] 15. Tests: same-period re-run is a no-op; next-period run adds exactly one bovenwettelijk
  slice; disabled config no-ops; unresolvable employee is skipped per REQ-ACCR-003/-ACCR-004/-ACCR-005
- [ ] 16. Test: a job-provisioned balance passes `nl-verlof-wettelijk-minimum` and
  `nl-verlof-vervaltermijn` through the real RuleEngine + `labour.json` corpus per REQ-ACCR-002
- [ ] 17. Quality gates: `composer lint` green, full PHPUnit suite green, SPDX + `@spec` tags on
