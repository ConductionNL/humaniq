...
- [ ] 1. Service: `lib/Service/ProformaPayslipService.php` — stateless `simulate()` that builds a
...
- [ ] 2. Service: input assembly per design.md D2 —
  ...
  REQ-PRO-001/-PRO-006
- [ ] 3. Service: employer-level knobs from `SettingsService::getPayrollAofTariff()` /
...
- [ ] 4. Service: the one-off bijzondere beloning is folded into the period gross as a combined-loon
  estimate (NOT bijzonder tarief), labelled as such in the returned breakdown; no new tax logic per
  REQ-PRO-006
- [ ] 5. Controller: `PayrollController::proforma()` (`#[NoAdminRequired]`) delegating to
  `ProformaPayslipService::simulate()`, returning the full breakdown JSON; 400 on malformed input per
  REQ-PRO-002
- [ ] 6. Controller: `authorizeProformaAccess()` — resolve-first capability probe over the hrmq payroll
...
- [ ] 7. Route: `appinfo/routes.php` — `POST /api/payroll/proforma` → `payroll#proforma`, BEFORE the
  SPA catch-all per REQ-PRO-002
- [ ] 8. Command: `lib/Command/ProformaPayslipCommand.php` (`hrmq:payroll:proforma` with
...
- [ ] 9. Manifest: new "Simuleer loonstrook" top-level `menu` entry routing to a `type: custom`
...
- [ ] 10. Frontend: host-app SFC for the `ProformaPayslip` custom page — the input form + the rendered
  breakdown, calling `POST /api/payroll/proforma`; Dutch labels, English i18n keys per REQ-PRO-005
- [ ] 11. Tests: `tests/Unit/Service/ProformaPayslipServiceTest.php` — the anchor input
...
- [ ] 12. Tests: assert `simulate()` performs no ObjectService write (persists nothing) — mocked/absent
  ObjectService, object counts unchanged per REQ-PRO-001
- [ ] 13. Tests: `tests/Unit/Controller/PayrollControllerProformaTest.php` — an HR caller gets the
  breakdown; a caller whose RBAC cannot resolve the payroll register gets 404; malformed gross → 400
  per REQ-PRO-002/-PRO-004
- [ ] 14. Quality gates: SPDX + `@spec` tags on every new PHP method (gate-16), i18n keys ENGLISH
