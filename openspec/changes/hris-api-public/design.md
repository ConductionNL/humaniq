...
(`apps-extra/openregister/`, not humaniq's own code — humaniq owns no controller in this area, ADR-022):
...
- `appinfo/routes.php:718-733` — the real, live route table: `GET /api/objects/{register}/{schema}`
  ...
  surface — it is the exact API the humaniq Vue frontend itself calls for every declarative manifest
...
- `lib/Controller/ObjectsController.php:1120` (`index()`), `:2415` (`show()`), `:2612` (`create()`)
  ...
  platform mechanism, not something humaniq or OpenRegister implements.
...
- `openspec/changes/humaniq-mcp-adoption/design.md` — "OpenRegister RBAC remains the authoritative
  ...
  confirmed by reading `humaniq-mcp-adoption`'s own scope: it edits `configuration.x-openregister-mcp`
  blocks only, never touches `ObjectsController` or RBAC configuration.
- `grep -rn "x-openregister-rbac\|rbac" lib/Settings/register.d/*.json` (humaniq's own worktree) — no
  match. RBAC grants are configured at the OpenRegister/Nextcloud-admin level (per-user/per-group
  ACLs on a register or schema), not declared inside any humaniq schema fragment. humaniq has no
...
- ADR-022 (apps consume OpenRegister's abstractions) and `openspec/config.yaml`'s own
  `rules.proposal` entry: "Humaniq owns no backend CRUD; HR objects live in the OpenRegister `hrmq`
...
been granted access, for what purpose, reviewed when — extending `humaniq-mcp-adoption`'s privacy
...
reviewed — exactly the gap `humaniq-mcp-adoption`'s own design.md would call out if it existed for
...
humaniq's schema fragments (`lib/Settings/register.d/*.json`) have no RBAC-declaring block (Context —
...
### D3 — Recommended schema subset reuses humaniq-mcp-adoption's AVG reasoning, not a fresh analysis

`humaniq-mcp-adoption` already did the hard work of classifying all 23 schemas against AVG art. 9
...
That reasoning is not specific to LLM agents — it is specific to "what may leave humaniq's boundary
...
Settings UI, never humaniq code — the same "Nextcloud already owns this layer" finding
...
| CRUD/filter/pagination for every humaniq schema | OpenRegister's existing `ObjectsController` | zero new code — the abstraction this change documents rather than duplicates (ADR-022) |
| RBAC enforcement | OpenRegister's existing engine, admin-configured | outside humaniq's schema fragments entirely (D2) |
| Recommended schema subset | README guidance text | not a schema constraint — cannot be enforced from humaniq (D2/D3) |
...
- **No enforcement means a misconfigured RBAC grant is invisible to this catalog** — if an admin
  grants broader access than `IntegrationAccount.grantedSchemas` states, nothing in humaniq detects
...
- **Recommending humaniq-mcp-adoption's six schemas as a REST default (D3) is a judgement call, not a
