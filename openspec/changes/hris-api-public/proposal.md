...
**plus** SCIM provisioning — a second, parallel API stack, hand-written inside hrmq. **Verified
against HEAD 2026-07-17, reading the actual sibling `openregister` app**
(`apps-extra/openregister/appinfo/routes.php` + `lib/Controller/ObjectsController.php`): hrmq
**already has** a general-purpose, RBAC-enforced REST API over every one of its 23 schemas, and it
is not aspirational — it is the same API the hrmq frontend itself calls. `ObjectsController`
...
fields even for a caller with read access) — the same boundary `hrmq-mcp-adoption`'s design.md
...
whatever purpose, can already call this API today, with zero hrmq code.
...
`hrmq-mcp-adoption` just established for the fleet's sharpest privacy case (BSN, IBAN,
...
hrmq documents this fact for an external integrator, and nothing lets HR/security see, in one
place, *which* external systems have been granted access to *what* — the same governance question
`hrmq-mcp-adoption` answers for LLM tool exposure has no answer at all for ordinary REST
...
- **Documentation only, for the API itself**: `README.md`/`docs/` gain a "Public HRIS API" section
  ...
  API enforces it server-side, including automatic `writeOnly` secret redaction). No new hrmq
  route, controller, or service — this section documents OpenRegister's existing surface as it
  applies to hrmq's schemas.
- **NEW `IntegrationAccount` catalog schema** (`lib/Settings/register.d/hr-integrations.json`) — a
  ...
  configuration, outside hrmq's schema fragments), `status` (enum `actief`/`ingetrokken`),
  ...
  reviewed when" — a question hrmq cannot answer today for the REST surface, exactly as
  `hrmq-mcp-adoption` answers it (via a hard allow-list, enforced) for the LLM tool surface.
- **A documented recommended read-only schema subset for new external HRIS integrations** —
  README guidance, mirroring `hrmq-mcp-adoption`'s own AVG reasoning (Art. 9 special categories,
  ...
  design.md D2 for why hrmq cannot enforce it in code.
...
- **No webhooks/event-streaming** — no event-dispatch infrastructure exists in hrmq for this
...
- **No custom API-key/scope/rate-limit system** — Nextcloud app passwords + OpenRegister RBAC
  already provide credential issuance and authorization; hrmq inventing a parallel credential type
...
- **`IntegrationAccount.grantedSchemas` does not enforce access** — it cannot: OpenRegister's RBAC
  configuration lives outside hrmq's own `register.d` fragments (no `x-openregister-rbac`-style
  block exists in any hrmq schema, verified by grep), so hrmq has no mechanism to *write* an
...
- `hris-api-public`: the `IntegrationAccount` governance catalog, the documented public-API
  ...
  recommended-subset guidance mirroring `hrmq-mcp-adoption`'s privacy discipline.

### Modified Capabilities

_None._ OpenRegister's `ObjectsController`, RBAC engine, and `RenderHandler` are read, not
modified — this change adds zero lines to any OpenRegister file. `hrmq-mcp-adoption` is referenced
...
- No `lib/Controller/`, `lib/Service/`, or `appinfo/routes.php` change — no hrmq API surface is
...
- Related: the superseded `spec/hris-api-public` draft branch is the source material; its REST v1/
  ...
  distinct, unbuilt capability, not silently dropped. `hrmq-mcp-adoption` (active) owns the
