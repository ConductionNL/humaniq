...
- [ ] 1. Tables loader: `lib/Payroll/TaxTables.php` — load + shape-validate
...
- [ ] 2. Calculator core: `lib/Payroll/PayrollCalculator.php` — the D2 chain (tabelloon,
...
- [ ] 3. Calculator variants: AOW-age switch (dateOfBirth vs tables' AOW-leeftijd, month
  granularity, birth-year bracket-set row, AOW korting columns + ouderenkorting) and groen =
  no-ARK path per REQ-PCE-002
- [ ] 4. Config: `payroll_aof_tariff` (laag|hoog, default laag) + `payroll_whk_percentage`
  (default = tables' flagged average) getters in `lib/Service/SettingsService.php` (design.md D2
  step 9)
- [ ] 5. Service: `lib/Service/PayrollRunService.php` — run probe/create for
  ...
  per REQ-PCE-003
- [ ] 6. Service: draft-only recalculation guard + human-approve preservation (never write any
  status but the initial draft) per REQ-PCE-004
- [ ] 7. Service: payslip stamping (payrollRunId, userId from nextcloudUserId, components incl.
...
- [ ] 8. Commands: `lib/Command/PayrollRunCommand.php` (`hrmq:payroll:run --period
  [--administration] [--recalculate]`, per-employee outcome output) + register in
  `appinfo/info.xml` per REQ-PCE-006
- [ ] 9. Commands: `lib/Command/PayrollVerifyCommand.php` (`hrmq:payroll:verify --period
  [--administration]`, RuleEngine over exactly the run+payslips, exit 0/non-zero) + register per
  REQ-PCE-006
- [ ] 10. Checks: `lib/Standards/Checks/NlEngineChecks.php` (both predicates, vacuous-scope
  guards, tables-dir glob at construction) + `RuleAuditService` `payroll.runsById` context
  enrichment per REQ-PCE-007
- [ ] 11. Controller: `lib/Controller/PayrollController.php` (`calculate`, `#[NoAdminRequired]`,
...
- [ ] 12. Manifest: PayrollRuns `open-form` "Loonrun aanmaken" action; PayrollRunDetail `api-call`
...
- [ ] 13. Fixtures: `tests/fixtures/payroll-2026/*.json` — anchor (must byte-match the design.md
...
- [ ] 14. Tests: `PayrollCalculatorTest` (all fixtures, table-driven) +
...
- [ ] 15. Tests: `PayrollRunServiceTest` (mocked ObjectService: idempotent probe, draft-only
...
- [ ] 16. README: the non-certification disclaimer section (rekenvoorschriften-based, NOT
...
- [ ] 17. Quality gates: `composer check:strict` green; in the dev container run
  `occ hrmq:payroll:run --period 2026-02`, inspect the generated run/payslip against the D2
  anchor, `occ hrmq:payroll:verify --period 2026-02` exits 0, tamper a nettoPay and confirm the
  non-zero exit, and `occ hrmq:rules:audit` shows both engine rules enforced with zero regressions
