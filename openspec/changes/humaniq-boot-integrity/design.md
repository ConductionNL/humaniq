...
**Goals:**
- Pin down, with live evidence, which of the three candidate layers (humaniq's provide / the
...
- Choosing humaniq's next release-versioning scheme. Decision 3 notes `package.json` vs
...
1. **humaniq's provide (`src/App.vue:74-80`) is correct.** `setup()` reads
   `loadState('humaniq', 'activeAdministrationId', '')`; when empty it provides
...
3. **Live reproduction, today, against `localhost:8080`:** navigated to `/apps/humaniq/employees` as
   ...
   **CORRECTED 2026-08-19 (orchestrator review) — the original evidence for this item read the
   ...
   evidenced by
   `document.getElementById('initial-state-humaniq-activeAdministrationId').textContent === ''`.
   ...
   ```
   <input type="hidden" id="initial-state-humaniq-activeAdministrationId"   value="IkFETS0wMDEi">      → "ADM-001"
   <input type="hidden" id="initial-state-humaniq-activeAdministrationMode" value="InN0YW5kYXJkIg=="> → "standard"
   ```
...
here and corrected per the brief's own instruction to fix stale figures): `js/humaniq-main.js`'s mtime
...
correct version of) the dependency drift in §7.** Not (a): humaniq's provide is correct as written.
Not (b): the library's resolver is correct as written, in both the checked-out repo and the
installed copy. humaniq's fix scope is therefore: get the dependency tree installed at the pinned
...
**Alternative considered and rejected:** patch around the defect locally in humaniq (e.g. resolve
...
humaniq utility). Rejected: the mechanism that resolves this correctly already exists and already ships
in the dependency humaniq already uses; a local workaround would duplicate it, diverge from it over
...
documents humaniq falling into once before (inventing `@administration` instead of using
...
into the current `js/humaniq-main.js` is `0.1.0` (grepped directly out of the built file). None of the
...
its kind. (This same weakness sits under `humaniq-manifest-boot-and-http-cost`'s proposed ETag scheme,
...
`fs.statSync('js/humaniq-main.js').mtimeMs` against that commit timestamp × 1000 and fails, naming
...
2. Whoever next has install permission in a non-locked-down checkout: run the checks (expect the
   ...
   correct the installs, re-run (expect PASS), rebuild `js/humaniq-*.js`, run the manifest-sentinel and
...
- Whether the content-hash sidecar (`js/build-info.json`) should also feed
  `humaniq-manifest-boot-and-http-cost`'s proposed ETag (replacing its `appVersion`-derived ETag with
