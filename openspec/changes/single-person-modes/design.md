...
- `lib/Service/RuleAuditService.php` / `lib/Command/RulesAuditCommand.php` — `occ hrmq:rules:audit` is the
...
- `/home/rubenlinde/hrmq-goal/nextcloud-vue/src/schemas/app-manifest-v2.schema.json` — `$defs/visibleIfCondition`
  ...
  manifest.json` in this repo returns **0** — hrmq has never used this primitive; `PageController::manifest()`
...
- `/home/rubenlinde/hrmq-goal/nextcloud-vue/src/schemas/app-manifest-v2.schema.json` `banner` widget
...
export. None of these compute a payroll figure or extend an existing hrmq engine; all are IB-aangifte tooling, a
different compliance domain, and out of hrmq's stated scope (`openspec/config.yaml`).
...
The FE half — exactly how hrmq's `App.vue`/`CnAppRoot` boot sequence merges an `IInitialState`-seeded value into
`effectiveManifest.runtime.user` for `visibleIf` to read — is **not** fully traced here; `App.vue` already
performs the analogous `loadState('hrmq', 'activeAdministrationId', '')` → `cnWorkspaceContext` seed for
...
`loadState`-sourced merge) — this is the first real consumer of `visibleIf` in hrmq, so no second example exists
...
this codebase's `ObjectService`-mediated save path (ADR-022 — hrmq owns no CRUD) does not currently expose for
...
hrmq:rules:audit` run — visible, traceable, never silently wrong — but never blocks anything. Reversing the mode
...
**The manifest surface.** A `banner` widget (`variant: warning`, `visibleWhen: {endpoint: '/apps/hrmq/api/payroll/
...
1. **`ADM-001.mode` stays `standard`** (no change) — proves the default is a no-op for every existing seeded
   multi-employee fixture; `npm run check:manifest` and the existing `occ hrmq:rules:audit` output are unaffected.
2. **A NEW `ADM-003` Administration**, `mode: dga_single_person`, with exactly one seeded `Employee`
   ...
   self-service banner renders the warning variant; `occ hrmq:rules:audit` still reports the same
...
- **`visibleIf` is genuinely new infrastructure in hrmq** (Context: 0 existing usages) — D2's FE wiring carries
...
- **The headcount check (D4) is soft by design** — a `dga_single_person` administratie that grows to two
  employees is flagged, not blocked, on the next audit run; between drift and the next `occ hrmq:rules:audit`,
