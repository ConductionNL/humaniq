...
humaniq detail pages, in production, for every timesheet and every expense claim — not a hypothetical,
...
fields. humaniq ships a widget that *advertises* consuming that abstraction (the manifest `_note`
...
### New Capabilities
- `humaniq-employee-relations`: `Timesheet`/`Expense` objects carry a resolvable OpenRegister relation
