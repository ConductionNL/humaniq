...
- [ ] 1. Schema: `LeaveTransaction` appended to `lib/Settings/register.d/hr-leave.json`
...
- [ ] 2. Guard: `lib/Lifecycle/LeaveBuySellApprovalGuard.php` on `approve` — delegates to
...
- [ ] 3. Guard: `lib/Lifecycle/LeaveSettlementPeriodGuard.php` on `settle` — fail-closed unless
  `settlementPeriod` present and `YYYY-MM`-shaped, stateless (`CompEffectiveDateGuard` shape) per
  REQ-BUYSELL-004
- [ ] 4. Rule: `nl-verlof-bovenwettelijk-niet-negatief` in `lib/Standards/rules/labour.json`
  (`LeaveBalance.bovenwettelijkHours >= 0`, mandatory) + `RuleCatalogue::VERSION` bump per
  REQ-BUYSELL-003
- [ ] 5. Check: new predicate in the existing `lib/Standards/Checks/NlLeaveChecks.php` implementing
  the D3 rule, vacuous-scope-free (LeaveBalance is always in scope) per REQ-BUYSELL-003
- [ ] 6. Service: `lib/Service/LeaveBuySellSettlementService.php::settle()` — idempotent
...
- [ ] 7. Service: `lib/Service/PayrollRunService.php` gains
  ...
  REQ-BUYSELL-005
- [ ] 8. Schema: nullable `leaveBuySell` on `Payslip` (`lib/Settings/register.d/hr-objects.json`,
  mirrors `retroAdjustment`) + version bump per REQ-BUYSELL-005
- [ ] 9. Command: `lib/Command/LeaveBuySellSettleCommand.php` (`hrmq:leave:settle --id
  <transactionId>`) + register in `appinfo/info.xml` per REQ-BUYSELL-004
- [ ] 10. Controller: `settle` action (`#[NoAdminRequired]`, RBAC-resolve-first → 404,
...
- [ ] 11. Manifest: `EmployeeDetail` gains `emp-leave-transactions` FK-scoped object-list row
  (after `emp-comp-adjustments`, before Files, which shifts down) per REQ-BUYSELL-006
- [ ] 12. Manifest: new `LeaveTransactionDetail` page — data widget (exclude
...
- [ ] 13. Manifest: new self-service index `MijnVerlofKopenVerkopen` (`filter: {userId: "@me"}`)
...
- [ ] 14. Tests: `LeaveBuySellApprovalGuardTest` (self-approval denial via delegation, sell
  sufficiency allow/deny, buy always allows, unresolvable balance denies) per REQ-BUYSELL-001/-002
- [ ] 15. Tests: `LeaveSettlementPeriodGuardTest` (missing/malformed/valid `settlementPeriod`) per
  REQ-BUYSELL-004
- [ ] 16. Tests: `LeaveBuySellSettlementServiceTest` (mocked ObjectService: idempotent re-settle,
...
- [ ] 17. Tests: `PayrollRunServiceTest` extension (settled transaction folds into `leaveBuySell` +
...
- [ ] 18. Quality gates: `composer lint` + full PHPUnit suite green, `npm run check:manifest`
