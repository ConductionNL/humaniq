# Delta — jurisdiction-packs

A jurisdiction becomes uploadable configuration: an ordered step-DSL a pure interpreter executes,
plus a named PHP escape hatch for genuine national exotica (ADR-101). NL is re-expressed as the first
pack and must stay behaviour-identical — that is the only available proof the abstraction is real.
