# Design — hrmq-mcp-adoption

Context: ADR-063 (MCP as Platform Abstraction, hydra #102) + the fleet MCP wave, 2026-07-13.
hrmq at HEAD `274c5d3` has **no** MCP provider, **no** dialect, **no** `IMcpScannableServices`
...
root. hrmq's schemas live in `lib/Settings/register.d/*.json` fragments. Four of the six
...
- You cannot expose a **holiday balance** without also exposing a **sick-leave balance**,
  because hrmq models both as `LeaveBalance` rows distinguished only by the *value* of
  `leaveType`.

Both of hrmq's most-wanted assistant reads are therefore blocked — not by policy squeamishness
...
verb anywhere in hrmq.

| # | Schema | Fragment | Verbs | Search filters (all verified real properties) | Why it is safe *and* worth exposing |
|---|--------|----------|-------|-----------------------------------------------|--------------------------------------|
| 1 | `Vacancy` | `hr-ats.json` | search, get | `status`, `department` | Job postings are **published** by design (`title`, `description`, `department`, `status`, `closingDate`). Zero personal data — the applicants live on the excluded `Application`. "Which positions are open?" is the one hrmq question with no privacy cost at all. |
...
guarded by `OCA\Hrmq\Lifecycle\NoSelfApprovalGuard` — a separation-of-duties control that
...
queries are unanswerable from hrmq's MCP surface *by construction*, and that is the design:
...
hrmq is, materially, a processing register of employment data — the most asymmetric
...
1. **Art. 9 — special categories.** Health data may be processed only under a narrow
   exception, and an employer may not even record the *nature* of an illness. hrmq's
...
2. **National identifiers and financial data.** The `bsn` is regulated beyond the AVG's
   ...
   `grossMonthlySalary`. A single `hrmq.Employee.get` would place all three in an LLM context
...
- `python3 -m json.tool` on each of the five touched `register.d` fragments after every edit;
  schema count across hrmq MUST stay 23 and no `x-openregister-lifecycle` block may move.
...
1. **A safe employee-directory read is the biggest gap.** "Who is X, what team, when did they
   ...
   exclusions in docudesk too; or (b) hrmq adds a curated
...
2. **Split the holiday balance from the sick balance.** `LeaveBalance` and `LeaveRequest`
   are contaminated by the `sick`/`care` values of the shared `leaveType` enum. If hrmq
