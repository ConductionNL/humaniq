 ...
 * `humaniq`. Every schema this app already owns still carries
 * `application = 'hrmq'`, so the pair matches nothing — and
 ...
 *
 * Measured on a live install before this step existed: 2 schemas still under
 * `hrmq` and zero under `humaniq`, and neither slug collides — so both move.
 * The gap is fleet-wide, not specific to one app — 231 under `docudesk`, 200
 * under `openconnector`, 180 under `procest`, 98 under `decidesk`, 21 under
 * `softwarecatalog`, 17 under `openbuild`, 2 each under `planix` and
 * `larpingapp`.
 *
 ...
 * migration, so this step logs and leaves both alone. Measured elsewhere in
 * the fleet: `openconnector` has 200 such collisions, `planix` and
 * `larpingapp` 2 each.
 *
 ...
 *
 * @spec exclude No canonical spec covers the `hrmq` -> `humaniq` schema
...
class MigrateSchemaApplicationId implements IRepairStep {
	 ...
	 * @var string
	 */
	public const NEW_APP_ID = 'humaniq';
	...
	public function getName(): string {
		return 'Move Humaniq schemas onto the humaniq application id';
	}//end getName()
